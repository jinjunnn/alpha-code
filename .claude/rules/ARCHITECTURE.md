...
## 硬性约束(不可协商)
1. **上游源码只读(fork,只增不改)**:本仓库是 `anomalyco/opencode` 的 fork(**非 submodule**,ADR-005 已取代 ADR-001);`packages/{opencode,core,server,app,ui,tui,sdk}` 等上游文件**只读**,永不修改。升级 = `git merge dev`(`dev` 为上游纯镜像)+ 契约 diff 适配。CI 守卫(`alpha-ci.yml` north-star guard):`git diff --diff-filter=DMR origin/dev...HEAD -- packages/{opencode,core,server,app,ui,tui,sdk}` 非空即红。**注:此后端只读铁律不受 ADR-016 前端接管影响**(前端接管是**新增** alpha 文件,不改上游源码)。
