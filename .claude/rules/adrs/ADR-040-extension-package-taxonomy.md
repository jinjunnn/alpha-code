...
3. ⚠️ **2026-08-03 owner 裁决：Phase 3 的本地 intake 是过渡形态，等发布端适配器成熟后退役。**
   ...
   **本 ADR 因此撤回原先「不裁决如何收敛」那句** —— 收敛方向已定，只剩排期。
   ...
   > ⚖️ **2026-09-17 owner 裁决（补注）：本地导入换芯与 OpenAI/Codex 转换器不做，本地 intake 退役无限期搁置。**
   > 上一句「只剩排期」与下文方案 B 里「本地导入改成跑同一份 vendored 适配器」**不再排期**。
   > 本条原文保留，作为当时的裁决依据，**不代表在推进**：
   >
   > - `alpha-code#715`（本地导入改用发布端同一份转换规则）、`alpha-code#215`（REQ-034 安装期适配器与本地导入）、
   >   `alpha-web#96`（OpenAI/Codex 插件适配器）已关闭为不计划；
   > - REQ-128 父需求 `alpha-work#49` 已按改写后的剩余 AC 验收关闭（AC2 的发布端适配器只保留 Claude Code，AC8 去掉本地导入一段）。
   >
   > ⇒ **现状路径继续有效**：装本机插件仍走下文登记的本地窄竖线（只装 skills，不走信封/admission）；
   > 签名目录一侧由已交付的 Claude Code 适配器（`alpha-web#98`）承载。发布端与本地各有一套转换规则的现状保留，
   > 下文登记的「映射被写两遍」就是这个被保留的现状。
   > 重开条件见各票关闭评论：确实要在 Alpha 里整包使用本机 Claude 插件的 agent 或外部工具服务时，按
   > [外部包适配方案基线](../../../docs/design/2026-08-04-external-package-adapter-baseline.md) §5/§8 重写 `#715` 再排期；
   > 出现真实的 OpenAI/Codex 插件发布者或用户时重开 `alpha-web#96`。
   ...
   **REQ-128 Phase 3（本地 Claude 插件导入）不在 Bundle 这条路上** ——
   ...
   ### ⚖️ 2026-08-04 owner 裁决：怎么收敛已经定了 —— **B 通往 A**

   上一版这里写着「本 ADR 不裁决如何收敛，但要求动 `#96`/`#98` 之前先回答」。**现在答了。**

   四个候选与它们的代价：

   | | 做法 | 判决 |
   | --- | --- | --- |
   | **A** | 本地 intake 直接退役，只留签名 catalog 一条路 | **终点**，但不能一步到位 —— 直接砍掉会让用户失去「装本机插件」这个今天已有的能力 |
   | **B** | 适配器抽成**纯库**（无网络、不写盘，输入 = 规范化文件表，输出 = 声明 + 兼容报告），发布端产物化；alpha-code **vendored** 它；本地导入改成「读本机目录 → 跑**同一份** vendored 适配器 → 拿到声明 → 走本地安装路径」 | ✅ **采纳** |
   | C | 本地导入只取字节，把目录发到 `alpha-web` 换回声明 | ❌ 装本机插件要联网，且把用户本地文件发到我们服务器。离线与隐私两头都坏 |
   | D | 两处各写一份，用同一份语料 + 差异闸钉住 | ❌ 违反本条本身；且仓内教训是「两份别人文法的替身一定会分叉」 |

   **B 与 A 不冲突 —— B 就是通往 A 的机制。** 映射先合成一份，本地那条路只剩
   「**取字节 + 调用**」这层壳；**壳什么时候拆掉是排期问题，不是设计问题。**

   由此定死的三件事：

   1. **provider 语义映射只有一份实现，物理上就是同一份字节**（发布端产出、宿主 vendored，
      两个方向的 vendor 搬运机制今天都已存在且带字节锁）⇒ **漂移结构上不可能**，
      不靠「两边都记得改」这种纪律。
   2. **适配器必须是纯的**：无网络、无子进程、无 `dynamic import`、不写盘、不求值模板、
      不跑 package script。输入是**已 capture 的规范化文件表**，输出是声明 + 报告。
      不纯就 vendored 不过来，B 当场退化成 D。
   3. ⚠️ **B 需要一条「接受声明但没有签名信封」的本地安装路径** ——
      它会碰到 owner 此前裁的「不碰已冻结的信封合同」那条边界。
      **`aw#96`/`#98` 的方案基线必须给出这条路径的确切落点与它对信封合同的影响面**，
      这是基线的准入条件之一，不是实现期再想的事。
...
6. ⚠️ **「Bundle 是唯一形态」今天接不住的，不是**容量**，是**种类**。**
   ⚠️ **本条初稿写错了一个承重数字，2026-08-04 由 `#826`（PR #836）的实测更正，并经第二条独立轴复核。**
   测量规则、逐插件分布与复算方法见
   [`docs/architecture/claude-plugin-corpus-component-scale.md`](../../../docs/architecture/claude-plugin-corpus-component-scale.md)。

   **更正**：初稿写「**7/62 组件数超过 16**（最大 22）」。那个数是在**另一个口径**下量的
   （把 `commands` 与 `hooks` 也算成组件、整份 `.mcp.json` 算 1 个）；在那个口径下它是对的，
   **但那个口径不是信封用的那个**。

   **按今天合法的四个 profile**（`skill` / `agent` / `mcp-local` / `mcp-remote`，
   `.mcp.json` 里**每个 server** 各算一个组件）逐插件实测：
   ...
   | **最大**组件数 | **13** |
   | **中位数** | **2** |
   | 合计 | **221** = 159 skill + 43 agent + 10 mcp-local + 9 mcp-remote |

   ⚠️ **但「0/62」比它看上去脆弱得多，不要据此认为容量够用。**
   最大那个插件的 `.mcp.json` **不是合法 JSON**（缺一个逗号、缺一个右括号），今天按 0 server 计；
   那份文件的字节里躺着 **12 个 https server** ⇒ 上游一旦修好，它就是 `13 + 12 = 25` 个组件，
   **直接越过 16**。**今天的余量是 3，而且靠一个第三方的语法错误撑着。**
   ⇒ `#827` 的题目随之改写：不是「抬上限因为装不下」，而是「**定一个有依据的界，并显式回答 25 怎么办**」。

   **真正的硬缺口在种类，而且比初稿写的大**：
   - **40/162 的技能是多文件**（最多 18 个文件），而 `skill` profile 的 payload 只有**一个** markdown asset
     → `#828` 已实现（宿主侧），待跨仓搬运；
   - **`commands` 没有任何 profile** —— **22/62 个插件带**（≈35%），共 100 个文件，实测 100% 是 `.md`、可执行位 0；
   - **`hooks` 在引擎里不存在这个概念** —— **12/62 个插件带**（≈19%），实测是 mode 755 的 `.sh`/`.py`。

   ⇒ **初稿「接不住约 11%」这个说法一并作废**：容量那一格今天是 0%，而 `commands` / `hooks`
   两格按插件计分别是 ≈35% 与 ≈19%。**缺口比原先写的大，而且落在另一个维度上。**
   这两格正是本 ADR **判据四**（「只有出现宿主装不了的新种类时才加 profile」）所设想的情形。

   **另一条会直接咬适配器的实测**：`.mcp.json` 有**两种摆法** —— 9 份带 `mcpServers` 包裹层
   （其中 2 份是空的）、**12 份 server 直接摆在顶层**（全部来自官方 `external_plugins/`）、1 份非法 JSON。
   **只读 `mcpServers` 的实现会把 19 个 server 数成 7 个** —— 又一次「手写别人文法的替身」。

   ⇒ **在补齐种类之前，`alpha-web#96`/`#98` 动笔就会撞墙**（撞的是 commands/hooks，不是组件数）。

   **本条自身的教训**（比数字更重要）：初稿那个数来自一次子代理回报，**没有第二条独立轴复核就被写进了 ADR 当硬事实**，
   并据此定位了整条地基的题目。本仓记录在案的「**前提为假的闸门比没有闸门更贵**」在这里的形态是：
   **前提为假的「问题定位」，会让三张地基票去修一个今天并不存在的毛病。**
   ⇒ **纪律：任何进入 ADR 的承重数字，必须有两条互相独立的测量轴，并写明口径。**
