...
- **(a) 远端逐工具禁用机制裁决(#490)**:引擎 `ConfigMCPV1.Remote` 只支持**整 server** enable/disable,**不支持** remote MCP 的逐工具 allow/deny。选定实现 = 不动上游、不整 server 关(会误伤兄弟),改用引擎**全局 permission 层**按工具 ID 过滤:`cloud-web-search.ts` 在 `OPENCODE_CONFIG_CONTENT` 注入 `permission[CLOUD_WEB_SEARCH_TOOL_ID] = "deny"`,把该 ID 从普通 + code-mode 模型工具集剔除。**该 ID 是引擎注册 id `cloud_cloud_web_search`,不是远端工具名 `cloud_web_search`(2026-07-31 更正,#650)**:引擎按 `McpCatalog.toolName(server, remote)` = `sanitize(server)+"_"+sanitize(remote)` 拼名,而云 worker advertise 的远端名本身就叫 `cloud_web_search`,拼上 alpha 的 server 名 `cloud` 之后是 `cloud_cloud_web_search`(#643 P2.1 实测 `calledTool`)。本条从 #490 落地到 #650 之间写的是远端名,经 `Wildcard.match` 编成 `^cloud_web_search$`,对真实 ID 恒不匹配 —— 这道 deny 在此期间是**空闸门**。ID 现由 `cloudMcpToolId()` 推导,不得再写第二份字面量,**保留** cloud server 与兄弟工具。**残留(loud,已留 TODO)**:远端 catalog 的 `listTools` **仍列**该工具,仅模型工具集不含它;真正的 remote-MCP 逐工具 deny 待引擎暴露该能力后替换。**禁**静默整 `cloud` server 关。
  ...
  **不是「逐字段完整覆盖」(2026-07-26 R7 措辞更正)**:真实 `mergeDeep` 探针显示继承来的 `headers` / `timeout` 会**留下**,`oauth` 子对象则被 `false` 整体替换。由于 `url` 已被换成不可用的 loopback,残留字段不能恢复原远端连接,故判决不变 —— 但宣称必须写成「覆盖连接控制字段」,不能写成逐字段完整覆盖。
...
- **(b) `OPENCODE_EXPERIMENTAL` umbrella 只收口 web_search 路径、不盲目 force-0(#490)**:umbrella 是 references / background-subagents / lsp-tool / plan-mode / code-mode / event-system / workspaces / oxfmt 等**全部**实验能力的总开关,盲目 force-0 会连带关掉它们(违反「一开关一具名能力」)。故交付实现**只** force-0 上列 4 个 keyless 专用 flag,**不碰** umbrella。**残留(open)**:`webSearchEnabled = providerID==="opencode" || enableExa || enableParallel`,其中 `enableExa = OPENCODE_EXPERIMENTAL || OPENCODE_ENABLE_EXA || OPENCODE_EXPERIMENTAL_EXA`——用户若显式 `export OPENCODE_EXPERIMENTAL=1`,该 OR 项在 env 层**压不掉**,本地 web_search 仍可复活。该闸(`registry.ts`)在 `UPSTREAM_PATHS`(north-star 守卫),**alpha 不可直接 patch**。收口该残留的两条路:**(i) alpha 可达**——对本地 web_search 工具 ID 也走 permission 层 `deny`(镜像 (a) 的云做法,零改上游);**(ii) 需 alpha-patch 机制**——patch `webSearchEnabled` 的 umbrella→enableExa 路径。
  ...
  **2026-07-26 六次收口(#223 R6 Blocker;上一段「拒绝**任何** MCP 工具中的 web-search 形态」与「例外 = 名字经 `ALPHA_CLOUD_MCP_SERVER` 过河」两句都**不成立**,作废)**:R6 实跑给出两条独立绕过,并据此把宣称收窄。
  ...
  - **证据**:`packages/ext/src/cloud-websearch-kill.test.ts` 的「R6 回归」组(四个合法改名命中、非 ASCII 名**登记为漏**、`cloud_attacker`/`cloud_web` 歧义、运行时新装 server、同名 add 残留)与「治理豁免绑定端点身份」组;`packages/opencode/test/tool/alpha-mcp-websearch-gate.test.ts` 的 R6 组走**真实**配置 + 真实 ext 装载 + 真 `Plugin.Service`,断言 `cloud_attacker_web_search` 被拒而权威云工具(引擎 id `cloud_cloud_web_search`,由 `McpCatalog.toolName` 推导)照常放行,以及「同名换 URL ⇒ 连它也被拒」。

  **2026-07-26 七次收口(#223 R7 Blocker;上一段的核验本身成立,但「核验完豁免即成立」在**多项目并存**时不成立,作废)**:R6 把核验结果存在 ext 的**模块级**变量里。插件模块由 Bun 动态 import **缓存**(一个引擎进程只有一份),而 Plugin / MCP 状态按 directory 建实例 —— 后跑的实例会覆盖先跑的快照。R7 只读探针实测:实例 A 记录正常 `cloud` 后 `cloud_web_search` 放行;实例 B 只要多记一个 foreign `cloud_web`,A 的**同一个**权威云工具立刻被误拒。一个项目因此能改变另一个项目云工具的可用性 —— 违反本票对治理云 server 的保证与 AC4「不误杀」。
...
- ✅ **已修 · #650(2026-07-31)**:云工具闸此前盯的是**远端**工具名而不是引擎注册 id(两层命名被混为一谈),后果两条 —— ① kill-switch 下的 permission deny 是**空闸门**,云 web search 从未从模型工具表消失;② ext 的云工具契约校验(`cloud-contract-hook.ts`,四个兄弟工具的 `CloudJobRequestV1` / `CloudJobAcceptedV1` / `CloudJobStatusV1` / `ArtifactListV1`)用精确相等比远端名,**一次也没执行过**。**AC4(kill-switch)未被击穿**:ext 的 `tool.execute.before` 闸按工具**形态**判(`isWebSearchToolId`),字面量无关,真实 id 照样命中。修法:ID 一律由 `McpCatalog.toolName(server, remote)` 的同义拼法推导(ui-mac `cloudMcpToolId()` / ext `mcpEngineToolId()`),判据测试改用**真的** `McpCatalog` + 真的 `Permission.fromConfig`/`disabled` —— 手喂 id 的断言正是让这个缺陷全绿活下来的原因。
- ⚠️ **残留(a)**:云工具 catalog `listTools` 仍列远端名 `cloud_web_search`,仅模型工具集不含——待引擎 remote 逐工具 deny。kill-switch 下调用一律被 ext 钩子拒(可见性是 cosmetic);**但**平台侧仍会接受并计费一个绕过引擎工具循环、直发的 `web_search` 调用 —— 服务端按 kill-switch 拒绝需要平台改动,不在本仓。
...
- ✅ **残留(b)已收口(2026-07-25 二轮 + 2026-07-26 三轮,#223)**:`export OPENCODE_EXPERIMENTAL=1` 的 env 层绕过由两层堵死 —— permission 注入(把工具从模型工具表滤掉)+ **工具自身的最终闸**(`ALPHA_LOCAL_WEBSEARCH_DENY`)。**只有第二层是主权保证**:R2 动态复现,单靠 permission 注入会被 agent wildcard / 持久 session permission / approved 三条后置规则覆盖(见裁决 (b) 的二次收口段)。
  ...
  **六轮更正(2026-07-26,#223 R6)**:五轮把那个钩子说成「关掉**一切** MCP web search」并把例外建在**名字前缀**上 —— 两句都不成立(合法改名与非 ASCII 名分类不出来;`cloud_attacker` 能冒领豁免)。例外已改绑**端点身份**,分类扩宽但**降级为尽力而为**;保证范围正式收窄为「alpha 治理的云 server + alpha 自有的本地 websearch 工具」,第三方 web-search MCP **不在**保证内(归 owner 复核的产品语义仍在,但不再写成已实现)。同轮修掉的 Major:kill-switch 下继承自 global/alpha.jsonc/项目的同名 `cloud` 定义以前**不会**被抹掉,现由中和条目覆盖其**连接控制字段**(`type`/`url`/`enabled`/`oauth`;`headers`/`timeout` 留存但已无处可发,见裁决 (a) 的 R7 措辞更正)(managed/MDM 通道未闭合,已诚实登记)。见裁决 (a)/(b) 的六次收口段。
