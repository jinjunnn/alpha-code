...
- 修订:2026-06-24 — 新增**浏览器自动化连接器**(backlog E14):catalog 加 `mcp:playwright`(`npx -y @playwright/mcp`,Apache-2.0,Microsoft 官方,`category:dev`),补 `webfetch` 抓不动 JS 动态站点的能力缺口。**零代码新增**——安装/预检/持久/启停链路全复用既有(`npx` 已在命令白名单)。决策:本地 `npx` 优先 + **仅定制中心可装(不进 `injectAlphaConfig` 预设)**。实测定论(2026-07-22;证据:[D5 Playwright-kernel 实测](../../../docs/verification/2026-07-22-d5-playwright-kernel/verify.md)):默认无 `--browser` 参数时使用系统 Chrome,首次 navigate 下载 0 字节;显式 `--browser chromium` 时首次 navigate 从 `cdn.playwright.dev` 下载约 268 MB;系统无 Chrome、下载或启动失败均响亮报错,不会静默回退或伪装成功。默认 `installSpec.command` 与 `runtimeDep` 确认保持原样。中国区弱网格(需 `sudo pfctl`)与无 Chrome 安装态(需 Chrome-less box)的真机确认仍由用户后续补测。**给用户的浏览器面板(Phase B)不在此 ADR,单独走定位关。**
