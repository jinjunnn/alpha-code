// #1114 artifact-memory-gate 专用 origin —— **逐字拷贝**自
// docs/verification/2026-08-25-req092-402-artifact-transfer/probes/origin-raw.ts(除本注释块)。
// 拷贝而不 import:那边是冻结的取证证据,不能因为闸要演进而被迫可变;这边是闸的组成部分。
// 为什么必须是裸 socket(而不是 Bun.serve / node http)以及它曾经怎么骗过一轮测量,
// 见原文件抬头与取证 README §1。闸在每次运行时都会先对本文件标定(run.ts §calibrate),
// 标定不过 ⇒ 本次测量作废。
//
