...
| `~/.alpha/` | 全局安装物(installs.json 账本、skills/agents/plugins)、自动化任务与状态 | 全删 |
| `~/.opencode/` 内 **alpha 自有 symlink** | `.alpha` 桥接链(kind 级整目录链 + 条目级链,目标解析进 `~/.alpha` 才算) | **只摘链**;你自建的真实文件/目录、指向别处的链一律不碰 |
...
| `~/.opencode/opencode.jsonc` | 引擎共享配置。经定制中心装过连接器/插件的,**建议卸载前先在定制中心逐项卸载**(会净除对应条目);否则清除后文件里会留下指向已删 `~/.alpha/plugins` 的悬空引用,需手动删除对应 `mcp.*` / `plugin[]` 条目 |
...
```bash
du -sh ~/Library/Application\ Support/alpha-code ~/.alpha ~/.local/share/opencode 2>/dev/null
# 期望:全部 No such file or directory
ls -la ~/.opencode 2>/dev/null   # 只应剩你自建的内容与 opencode.jsonc(见上表)
```

保留项(设计内不归零):各项目 `.alpha/`、`~/.opencode` 用户自建内容与 opencode.jsonc、钥匙串项。
