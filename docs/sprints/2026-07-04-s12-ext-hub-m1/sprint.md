...
| T3 | 存量迁移弹窗(检测根 A alpha 写入物 → 清单确认 → 搬移+receipt)+ **A2 存量钉版迁移同场**;⚠️ 弹窗放行门 = T8 解 R3(代码可合、开关后开) | REQ-018 T4 + A2 尾项 | fable | ✅ **PR #71**:`alpha-migrate.ts`(scanLegacy 只报旧 XDG 根、removeLegacy 只删旧位,8 单测)+ IPC;renderer 迁移条(catalog 名字匹配=只迁 alpha 自装、不碰用户自建 ADR-019 §4;复用既有 installer 重装→**顺带 A2 钉版 + secret file 化**);**门控 `ALPHA_MIGRATE_ENABLE=1`**(默认关,A6 真机验证后由 T8 开);266 pass |
| **Track β —— 生效与密钥** | | | | |
