...
| REQ-071 | 默认用户工作目录 `~/Alpha`:用户数据有可见默认落点,不再只有隐藏 `.alpha`(全新用户零选择开始、选择器 defaultPath、无项目态产物归宿 + 目录契约/写入治理) | feature | A | in-sprint(S33) | **用户直提(2026-07-09),同日两轮 7 点全清**:①lazy 建 ②未选项目自动落(chip 默认)③无项目态自动化/云产物也落 ④目录名固定英文 `Alpha` ⑤子目录英文 Journal/Memory/Outputs ⑥云可见副本归 T2 ⑦总结=自动化模板+skill 约定(**配套新增内置技能 `alpha-workspace`**,不做专门 UI 入口);架构载体 [ADR-025](../.claude/rules/adrs/ADR-025-user-workspace-alpha-dir.md)(**accepted**):目录契约 + 写入者矩阵(主进程只供目录/skill 教约定/模板默认关/回流副本受控白名单);分期 T1 基础供给 / T2 契约配套;验收真源 [requirements/REQ-071](requirements/REQ-071-default-user-workspace-dir.md) |
