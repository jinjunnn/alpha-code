...
| REQ-009 | alpha-ci 提速:guard partial clone + bun 依赖缓存 | debt | A | ready | **CI 另议退回 ready(用户 2026-07-03)**;每个 PR 都在付的等待税;验收=单轮 ≤2min 且「改上游必红」用例仍红(需真 CI run 验);留 CI 域另批(独占 `.github/workflows/`);详见 [requirements/REQ-009](requirements/REQ-009-alpha-ci-speedup.md) |
...
| D12 | CI 卫生:上游 cron workflow 在 fork 误触 + lint gate + e2e 范围 | debt | A | ready | **CI 另议退回 ready(用户 2026-07-03)**;Actions 分钟燃烧 + 潜在误发布;含仓库设置 disable(需 gh);与 REQ-009 同批(独占 `.github/workflows/`) |
