...
| ID | 标题 | 类 | 仓 | 状态 | 备注 |
|---|---|---|---|---|---|
| REQ-047 | shell-env 探针把会话级隔离/调试 env 永久腌进缓存:安装物静默落死目录(placebo ok:true)+ ALPHA_CDP 被腌 = Finder 启动开调试端口 | bug | A | registered | **S27 真机批发现(2026-07-06,P0)**:`shell-env-cache.ts` 探针 spawn 登录 shell **继承 app 自身 env** → 批测/终端启动时的 `ALPHA_GLOBAL_DIR`/`OPENCODE_CONFIG_DIR`/`ALPHA_OPENCODE_HOME`/`ALPHA_MIGRATE_ENABLE`/`ALPHA_CDP` 被 `env -0` 回吐并写进 `alpha-shell-env.json`,此后每次启动(含 Finder)0ms 套用;**自续毒化**:套用→process.env 带毒→异步再探测又继承→缓存永不自愈(本批确定性复现 ×2,证据 [audits/vnext3](audits/2026-07-06-realmachine-vnext3/verify.md) + `req047-poisoned-shell-env-cache.json`);真实用户可踩(终端 export 过 OPENCODE_CONFIG_DIR 再从终端启动一次即中毒);修向=①探针最小干净 env(HOME/USER/SHELL/TERM/LANG)②读缓存 blocklist alpha/opencode 控制命名空间兜存量毒缓存③双向单测;本机已消毒(缓存删除 ×2) |
