...
| REQ-069 | 未登录模型默认/展示流程重设计:不默认 member-only 代理模型、海外 model id 不外显、默认链 = 上次使用(校验可用)→ 代理默认 → BYOK KEY → 空态引导 | ux | A | in-sprint(S32) | **用户真机报障(2026-07-08,4 截图)**:未登录启动默认选中平台代理模型 → 发消息即「预授权拒绝: member-only model 需 active 会员」原文抛给用户;picker 未登录仍逐条外显 GPT/Claude 海外 id(`alpha-composer-model.tsx:104` 全量只标 locked)+ 一处 UI 展示问题;REQ-056 的默认门(登录+proxyLive)是好的,漏洞在**持久化残留无可用性校验**(`composer-state.ts:42`)/引擎级默认两条旁路(实现时钉死);期望流程与空态设计已入档;验收真源 [requirements/REQ-069](requirements/REQ-069-logged-out-model-default-flow.md) |
| REQ-070 | endpoints/baseURL 迁移 custom domain:workers.dev → tidelabs.click(gateway/cloud 双端点,大陆可达性) | feature | X | in-sprint(S32) | **alpha-platform 审计 #3(2026-07-08)**:B 侧 PR #20 已合并、双域 live;A 侧改 `alpha-config.ts:21,27` 默认值 + 连带 alpha-code-plugin 仓 `.mcp.json`/README + C 侧 token endpoints 下发核查;完成确认后 B 才关旧域;**改号说明**:需求档原自编 REQ-067 与出厂治理撞号,按 ADR-018 改号;验收真源 [requirements/REQ-070](requirements/REQ-070-custom-domain-endpoints-migration.md) |
