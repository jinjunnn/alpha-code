...
| REQ-011 | 首页 composer 下方项目/会话 chips 移除 → 预留后续功能入口位 | ux | A | shipped | **PR #83**:删 recent pills 区(`a-home-recents`/`RecentPill`)+ now-dead hasProjects/newSessionHref/CSS;workspace chip + 左栏导航 + `useAlphaProjects` 取数不动;零改上游、322 tests 绿;**verified 待真机视觉**(布局不塌陷,→ REQ-016);「预留位放什么」仍在⚖️待拍板;详见 [requirements/REQ-011](requirements/REQ-011-composer-project-chips.md) |
...
| REQ-021 | 自动化(定时任务)完整需求:A1 本地只读 MVP → A2 增强 → A3 云档位 | feature | A | shipped | **A1 shipped(2026-07-04,PR #81,S15)**:调度器/执行链/readonly agent/一句话解析/侧栏面板全量;ADR-022 立(trial);隔离实例端到端实测触发+落盘(+2ms);**verified 待真机批**(到点+通知实拍/readonly deny 实测/重叠·睡眠·断电用例/回跳走查,并入 REQ-016);**A2/A3 未做留册**(A3 前置 REQ-022/B16);契约 [sprints/s15](sprints/2026-07-04-s15-automations-a1/sprint.md);详见 [requirements/REQ-021](requirements/REQ-021-automations.md) |
