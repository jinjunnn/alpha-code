...
| REQ-076 | Windows 平台支持:ship:windows 出包 + 平台差异收敛(platform seam)+ 安全/发布链双平台化 | feature | X | ready | **用户拍板(2026-07-09:「修改宪法,需要支持 win 版本」)**;宪法同日修订(POSITIONING/NON_GOALS#6/GLOSSARY/ARCHITECTURE/CLAUDE.md),方案 = [[ADR-026]](路径 `~/.alpha`/`~/Alpha` 全平台同构零特例 + platform seam 单点分发 + 共享/差异四层定义 + 安全诚实降级);技术底 = 全量可移植性审计([audits/2026-07-09-windows-portability-audit](audits/2026-07-09-windows-portability-audit.md)):**不需要改写**——上游本就发 Windows 版、打包段/feed/深链/WSL 通道现成、symlink 桥已退役 → 无硬崩点;阻断级仅 2(menu.ts 非 darwin 无菜单 / ext-ipc `which` 探测恒误报)+ 适配级 ~9(db-safety sqlite / 0600 NTFS 无效 / CSP 守卫 / 白名单等);~14-16 文件、核心 2-4 人日 + 等价 2-3 人日。分期 T1 出包冒烟 / T2 功能对齐 / T3 安全与发布(C 仓下载页+feed 配套);**归并 Parked D7**(重开条件「NON_GOALS#6 撤回」触发 → 关切入 T3);三决策点入待拍板队列;详见 [requirements/REQ-076](requirements/REQ-076-windows-support.md) |
