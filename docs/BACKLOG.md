...
| B20 | 弱网降级 UX:超时/重试/splash 状态/真骨架/websearch 优雅降级(S8) | ux | A | in-sprint | **S11 已收尾,余项转回 ready**;S11 T4 部分随 PR #60(splash 状态行 + banner/重试底座);余项=真骨架(Skeleton 死代码去留)/promptAsync 超时(豁免记录)/websearch(上游 R2 豁免),见 [audits/rescan](audits/2026-07-04-silent-failure-rescan.md) ⏭/🆗 行。**S19(2026-07-06)清最后可落码项**:Skeleton 死代码**删**(零引用实证);promptAsync/websearch 两项 🆗 豁免已记账 → 本项无剩余可落码工作,PR 合入即 shipped |
