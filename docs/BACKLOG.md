...
> 下一个新需求编号:**REQ-074**(新需求一律 REQ-NNN;A/B/C/D/E 为历史审计系列保留原号,用户 2026-07-03 确认)。
...
| ID | 标题 | 类 | 仓 | 状态 | 备注 |
|---|---|---|---|---|---|
| REQ-073 | 统一装配弹窗(@/+ 合一)+ 模式收编:重设计 @ 弹窗、移除 build 组件、计划模式收进弹窗以「⊗ 计划」chip 呈现(Codex 对标) | ux | A | shipped(PR #162) | **用户直提(2026-07-09,5 截图)**:现状 @/+ 两弹窗割裂(+ 弹窗四条扩展行全是同一 setExtHubOpen 动作 = placebo 分身)、AgentChip 信息量近零;方案 = 一个组件两触发器(添加/Agent/文件/扩展 四节,REQ-072 底座全复用)+ 去 AgentChip(默认缺省参数=引擎 build)+ 计划模式 toggle→chip(agent:"plan",Shift+Tab,会话级)+ 权限联动/第三方主档动态项;**7 点全拍(2026-07-09 同日)**:6 建议全同意 + ⑦内置 agent 显示但降噪(zh 简介+内置签);GLOSSARY 语法分工修订 v2(/ 执行 · @/+ 装配);**S33 shipped(2026-07-09,PR #162)**:设计稿 v1 过审(chip 左侧簇拍板)→ 统一弹窗四节 + 模式收编全量落地;顺手根治「重渲染合成 mousemove 劫持键盘选中」通用 bug(/ 菜单同受益);CDP 九场景核验(截图 sprint 档 cdp-073/);**残单**:打包后真机批;验收真源 [requirements/REQ-073](requirements/REQ-073-unified-assemble-popup.md) |
| REQ-072 | 斜杠命令菜单可用性重修:键盘导航失效/技能列表 12 条硬帽/搜索只匹配名字 三根因修复 + 类型 icon/来源标注/中文名/分组 展示重设计(Codex 参照稿对标) | ux | A | shipped(PR #161) | **用户真机报障(2026-07-09,2 截图)**:三根因登记时已源码钉死(`composer-autocomplete.tsx`)——①↑↓ 失效 = active 重置竞态(`:270` 每键 bump → `:214` items 变即 setActive(0),实现在而被自己覆盖)②技能不全 = `slice(0,12)` 硬帽(`:204`,引擎序非字母序)③搜索仅 `name.includes`(`:182`,描述不参与、无前缀优先、无结果菜单闪没);分期 T1 行为修复 / T2 展示重设计(类型 icon+来源收敛「内置/个人/项目/MCP」+出厂/catalog 补中文名、外来不机译);**S33 shipped(2026-07-09,PR #161)**:设计稿两版拍板全清(含追加:来源签行尾右缘/个人淡 indigo/选中淡底/B 案 `/agents` 单条入口 + `/`=执行 `@`=引用 语法分工固化 GLOSSARY);三根因修复 + 分节/icon/四档/中文映射/页脚全量落地,core 20 单测 + **CDP 六场景实机核验**(键盘 ↓×3 选中=3 实证,截图在 sprint 档 cdp/);**残单**:打包后真机像素批;验收真源 [requirements/REQ-072](requirements/REQ-072-slash-menu-usability.md) |
| REQ-071 | 默认用户工作目录 `~/Alpha`:用户数据有可见默认落点,不再只有隐藏 `.alpha`(全新用户零选择开始、选择器 defaultPath、无项目态产物归宿 + 目录契约/写入治理) | feature | A | shipped(PR #160) | **用户直提(2026-07-09),同日两轮 7 点全清**:①lazy 建 ②未选项目自动落(chip 默认)③无项目态自动化/云产物也落 ④目录名固定英文 `Alpha` ⑤子目录英文 Journal/Memory/Outputs ⑥云可见副本归 T2 ⑦总结=自动化模板+skill 约定(**配套新增内置技能 `alpha-workspace`**,不做专门 UI 入口);架构载体 [ADR-025](../.claude/rules/adrs/ADR-025-user-workspace-alpha-dir.md)(**accepted**):目录契约 + 写入者矩阵(主进程只供目录/skill 教约定/模板默认关/回流副本受控白名单);**S33 shipped(2026-07-09,PR #160,T1+T2 全量)**:lazy 供给(仅默认目录,文件占位诚实失败)+ chip/选择器/自动化/云任务四处默认接线 + 出厂技能 alpha-workspace + 每日总结模板(默认不开)+ Outputs 可见镜像(12 单测);**残单**:真机批(chip 默认/供给/技能生效/模板/镜像截图)→ verified;验收真源 [requirements/REQ-071](requirements/REQ-071-default-user-workspace-dir.md) |
...
| REQ-069 | 未登录模型默认/展示流程重设计:不默认 member-only 代理模型、海外 model id 不外显、默认链 = 上次使用(校验可用)→ 代理默认 → BYOK KEY → 空态引导 | ux | A | shipped(PR #159) | **S32 shipped(2026-07-08)**:model-default-core 纯核(27 单测)+ 挂起/恢复(登出残留暂停不删)+ picker 未登录收敛登录卡 + 发送 preflight;CDP 隔离环境双场景核验(全新未登录零泄漏零默认/登出残留挂起如实,截图在 sprint 档);**残单**:Image#4 UI 细节未定位(待用户截图/复现)+ 登录态默认不回归、BYOK 默认真机复验 → 真机批;**用户真机报障(2026-07-08,4 截图)**:未登录启动默认选中平台代理模型 → 发消息即「预授权拒绝: member-only model 需 active 会员」原文抛给用户;picker 未登录仍逐条外显 GPT/Claude 海外 id(`alpha-composer-model.tsx:104` 全量只标 locked)+ 一处 UI 展示问题;REQ-056 的默认门(登录+proxyLive)是好的,漏洞在**持久化残留无可用性校验**(`composer-state.ts:42`)/引擎级默认两条旁路(实现时钉死);期望流程与空态设计已入档;验收真源 [requirements/REQ-069](requirements/REQ-069-logged-out-model-default-flow.md) |
| REQ-070 | endpoints/baseURL 迁移 custom domain:workers.dev → tidelabs.click(gateway/cloud 双端点,大陆可达性) | feature | X | shipped(A PR #159 + alpha-web PR #15 + plugin 仓 b32c5eb) | **S32 shipped(2026-07-08)**:A 默认值 + 回归锁语义反转 + C 下发默认值(**核查证实 token 确实下发 endpoints,不改则 discovery 覆盖 A 默认**)+ plugin 仓两处;新域探活 /health `/v1/models` 200;**verified 待**:C 部署后真机登录+流式+cloud dispatch 经新域;**验收③(通知 B 关旧域)已从 DoD 剥离**——硬前置 = C 部署 + 存量 v0.1.x 升级率(旧默认烧进包,关旧域即砖化未升级客户端);**alpha-platform 审计 #3(2026-07-08)**:B 侧 PR #20 已合并、双域 live;A 侧改 `alpha-config.ts:21,27` 默认值 + 连带 alpha-code-plugin 仓 `.mcp.json`/README + C 侧 token endpoints 下发核查;完成确认后 B 才关旧域;**改号说明**:需求档原自编 REQ-067 与出厂治理撞号,按 ADR-018 改号;验收真源 [requirements/REQ-070](requirements/REQ-070-custom-domain-endpoints-migration.md) |
