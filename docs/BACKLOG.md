...
> 下一个新需求编号:**REQ-053**(新需求一律 REQ-NNN;A/B/C/D/E 为历史审计系列保留原号,用户 2026-07-03 确认)。
...
| ID | 标题 | 类 | 仓 | 状态 | 备注 |
|---|---|---|---|---|---|
| REQ-047 | shell-env 探针把会话级隔离/调试 env 永久腌进缓存:安装物静默落死目录(placebo ok:true)+ ALPHA_CDP 被腌 = Finder 启动开调试端口 | bug | A | verified | **S27 真机批发现(2026-07-06,P0)**:`shell-env-cache.ts` 探针 spawn 登录 shell **继承 app 自身 env** → 批测/终端启动时的 `ALPHA_GLOBAL_DIR`/`OPENCODE_CONFIG_DIR`/`ALPHA_OPENCODE_HOME`/`ALPHA_MIGRATE_ENABLE`/`ALPHA_CDP` 被 `env -0` 回吐并写进 `alpha-shell-env.json`,此后每次启动(含 Finder)0ms 套用;**自续毒化**:套用→process.env 带毒→异步再探测又继承→缓存永不自愈(本批确定性复现 ×2,证据 [audits/vnext3](audits/2026-07-06-realmachine-vnext3/verify.md) + `req047-poisoned-shell-env-cache.json`);真实用户可踩(终端 export 过 OPENCODE_CONFIG_DIR 再从终端启动一次即中毒);**shipped(同日,S27 快车道,PR #127)**:①探针净化 `minimalProbeEnv`(同步+异步两处 spawn 只带自举白名单+系统 PATH;缓存语义=「登录 shell 自己导出什么」,干净刷新对存量毒缓存整份重写自愈)②读侧 `sanitizeCachedShellEnv` 剥离 8 控制键 + `[req047]` loud 留痕(剥空→null 回退同步干净探测);+5 单测(500 全绿);真 export 场景不受影响(preferAppEnv 层照常);**verified 待**真机(毒缓存自愈 + Finder 启动零 9222)→ S27 下场次;本机已先行消毒(缓存删除 ×2)  **verified(2026-07-06,S27 场次二)**:重打包签名包毒化 alpha-shell-env.json(注 ALPHA_CDP=9222+两死目录)→ `open -a` 冷启动:读侧剥离 3 毒键(52 vars 计数差为证)+ `lsof -iTCP:9222` 零监听 + 异步干净探测整份重写(22 vars 自愈);二次冷启稳态零 9222;证据 [audits/vnext3 场次二](audits/2026-07-06-realmachine-vnext3/verify.md)。 |
| A2 | catalog MCP 全部钉精确版本 + 存量配置一键钉版本(T1.5) | security/perf | A | verified | **S12 完成**:catalog 全条目钉版本(2026-07-03)+ **存量一键钉迁移随 T3 迁移引擎落地(PR #71)**——迁移复用 catalog 已钉版本重装,旧位净除。**verified(2026-07-06,S21 真机批 B1/M1 迁移开门演练)**:`ALPHA_MIGRATE_ENABLE=1` + 隔离 legacy 种子 → hub 迁移条 → 迁移 → 四要件核验(`.alpha` 真源落地 + 账本 receipt + 旧位净除 + `~/.opencode` symlink 桥)全通([audits/s21](audits/2026-07-06-s21-realmachine-vnext2/verify.md));真实根迁移(用户自建内容风险)留用户批 → [[REQ-044]]。R3 澄清:catalog 钉版本≠推广安装、不扩 A6 面 |
...
| REQ-052 | 出厂技能绕过 `.alpha` 真源:`~/.opencode/skill/<name>` 直链 app 资源,破「`.opencode` 内 alpha 自有条目只指向 `.alpha`」不变量 | debt | A | registered | **用户点名(2026-07-07)**:环境重建时打开 `~/.opencode` 见 skill-creator/agent-creator 两条直链 app Resources 的 symlink(REQ-036 初版零拷贝捷径,跳过 `.alpha`),质疑 `.alpha` 真源纪律被破——目录安装链路合规,唯此通道漂移。修=改**两跳桥**与目录安装同构:`~/.alpha/skills/<name>`(真源 symlink→app 资源,保零拷贝)+ 复用 alpha-bridge 落 `~/.opencode/skills/`(多跳链引擎可见,REQ-004 spike 实测);启动 reconcile 自动迁移旧直链(仅 `isAlphaFactoryLink` 判我方链才拆,用户内容照旧不碰);不变量写入 ADR-019 修订 |
| B16 | 云派发 PIPL 数据出境同意/告知门(显式 per-项目 consent + 隐式登录告知) | security | X | verified | **S25 shipped(2026-07-06,用户 GO,PR #123 + alpha-web PR #9)**:①显式——hub 云派发 main 侧 gate(`alpha-cloud-consent.ts` 纯核 15 单测 + `cloud-ipc.ts` 原生同意门 + `.alpha/prefs.json` 落 cloudConsent,版本化,写盘失败不静默放行)②隐式——授权页平台代付告知行 + 隐私政策修正 §2 误导+增出境专章。**覆盖面=hub 派发**;MCP facade/云自动化路径不覆盖(残余如实入档)。**verified 待真机**(同意门实拍/拒绝/prefs 落盘/二次不弹);详见 [requirements/B16](requirements/B16-pipl-consent-gate.md)  **verified(2026-07-06,S27 场次二)**:云 code-review 派发首次弹同意门→接受→`.alpha/prefs.json` 落 cloudConsent(v1);二次派发同项目**不弹**、直接执行;拒绝路径未单独走查(接受即验主链)。 |
...
| B3 | 云协同最后一公里:cloud MCP 健康 → dispatch → 进度 → artifact 回流(=G4、E12;T4.1-4.3) | feature | X | verified | **S11 T2(PR #55+#58)**:回流全链落地——主进程 `alpha-workdir.ts`(`.alpha/runs/<runId>/`,防逃逸/消毒/体积帽)+ `cloud-save-run` IPC(#55);renderer `CloudRunWatcher`(firehose tool-part 终态检测 → saveRun → toast,worktree 映射,纯解析核 10 单测)+ i18n(#58);呈现=会话内工具调用(验收④,引擎原生);**verified 待真机**:登录态 in-app dispatch 冒烟(兼 REQ-004 verified);配额预估 UI(验收⑥后半)→ T4/B11 账户 banner 一并;**R1:勿切端点**  **verified(2026-07-06,S27 场次二)**:登录态 in-app dispatch ×2(job_27428a9aafa8/job_3830f092ae8d)→ job.completed → 产物落 `.alpha/runs/<id>/{status,contract}.json`;diff-only 坐实(contract.input 仅 diff 键、工作树 diff 非全库);REQ-020 云管线返回真实 code-review 结果 + 联网 citations。 |
...
| B6 | 装载 `@alpha-code/ext` 主接缝(=G1;T5.1-5.2) | feature | A | verified | **PR #46**;extraResources alpha-ext/ + StartCommand 传路径 + injectAlphaConfig 合并 V1 `plugin` 数组;ALPHA_EXT_DISABLE 逃生;**bundle 打包态加载已核(2026-07-05,REQ-016 S16)**:main.log `alpha-ext: loading plugin bundle` from Resources/alpha-ext/plugin.js(410KB);**verified(2026-07-06,S20 C2,S20 审计收尾补翻)**:登录态真会话 prompt「call alpha_ping」→ 模型最终输出逐字 = 工具输出格式(pong + directory + worktree)→ 工具进列表且真执行 = **G1 成功条件达成**(证据 [audits/s20 C2](audits/2026-07-06-s20-realmachine-vnext/verify.md));详见 [requirements/B6](requirements/B6-ext-seam-activation.md) |
...
| B14 | 会话 DB 备份/导出(损坏恢复) | feature | A | verified | **S17 T3 shipped(2026-07-05)**:备份引擎=readonly `VACUUM INTO`+必验(验不过即删,反 placebo——实证 `-readonly .backup` 静默假成功)+滚动保 5;自动触发=pre-migration 时点;手动入口=「数据」菜单(备份/导出…/打开文件夹,dev 置灰);损坏启动指向最近备份恢复(WAL 残件连带隔离);④ 同屏入口随 C16;**verified 待真机**(菜单实操+原生对话框);设计 [designs/db-safety-belt](designs/2026-07-05-db-safety-belt.md);详见 [requirements/B14](requirements/B14-db-backup-export.md)  **verified(2026-07-06,S27 场次二)**:菜单栏「数据 ▸ 立即备份会话数据库」→ 备份落 `<userData>/alpha-db-backups/*.db`(4.0MB),`PRAGMA integrity_check=ok`+20 表(readonly VACUUM INTO 必验产物有效)。 |
...
| REQ-032 | catalog 远程分发(收编 E10):C 端点+签名 + A 运行时拉取/缓存/回退 + skill/agent 远程资产通道 + 条目级更新检查 | feature | X | shipped | **S18 T10 shipped+prod(2026-07-05,C alpha-web #5 已部署 + A PR #103)**:C 静态端点(catalog.json+ed25519 .sig+不可变 assets,发布脚本拒改已发布版本)+ A 拉取链(ETag 304 实测/验签(篡改拒用实测)/缓存/回退 远端→缓存→内置)+ 远程技能安装(sha256 钉死,坏 sha 拒装实测,builtin 同管线桥+账本)+ 条目级更新角标(X7 origin 过滤);**远程-only 条目 conventional-commits 实证上架不发版**;E10 dup 并入;像素/回滚演练→残单;详见 [audits/s18-t10](audits/2026-07-05-s18-t10-req032/verify.md) |
...
| REQ-036 | 创建技能化:移除 hub 交互式创建表单,skill/agent 创建统一走技能(skill-creator 出厂化 + agent-creator + alpha_reload 生效闭环) | feature | A | shipped | **S18 T2 shipped(2026-07-05,PR #100)**:通道实测二分推翻原 env 注入设计(OPENCODE_CONFIG_CONTENT.skills.paths 引擎不生效)→ 改 `~/.opencode/skill` symlink 桥(ADR-019 同构,reconcile 幂等+逃生开关,7 单测);alpha_reload 两段式(X9 实锤流中 dispose 打断回复 → session.idle 后执行);表单删除+导入语义+出厂徽标;agent-creator 出厂技能;裸引擎决定性验证在册 [audits/s18-t2](audits/2026-07-05-s18-t2-req036/verify.md);像素/端到端→真机批;详见 [requirements/REQ-036](requirements/REQ-036-creation-via-skills.md) |
...
| REQ-046 | catalog 双作者源收敛:C 仓唯一作者真源 + A 内置改快照生成 + CI 守卫禁手编 | debt | X | verified | **S24 shipped(2026-07-06,PR #122 + alpha-web PR #8)**:①快照链——`sync-catalog-snapshot.mjs`(fetch+ed25519 验签(公钥单源)+版本单调拒回退+**字节原样**快照+meta)实跑 prod(2026-07-06.1/23 entries);守卫=`alpha-catalog.test.ts` sha256 断言,**红绿演练 PASS**(手编一字节→红,脚本刷新→绿);②agent 远程通道接线(installRemoteAgent 单 .md 约定+IPC/preload/renderer 全链,信任边界同远程技能,+5 单测)→ **「新增条目零发版」四类全部成立**(plugin=通道例外走 npm 发包,用户裁决);③ADR-023 修订(四类通道表+仍需发版三情形)+ DISTRIBUTION ①′ + C 侧 catalog-publish.md;**verified(2026-07-06,S27 真机批 T0+M1-3)**:C 上架首条远程 agent `bug-triage`(alpha-web PR #11,catalog 2026-07-06.3)→ 签名包 hub 详情页安装 → 真源 .md 字节一致 + receipt + agents 整目录桥(dispose 重扫,mode:subagent 经 task 委托可用)——「新增条目零发版」四类全部真机闭环([audits/vnext3](audits/2026-07-06-realmachine-vnext3/verify.md));原拍板/漂移证据见档 [requirements/REQ-046](requirements/REQ-046-catalog-dual-source-guard.md) |
...
| REQ-014 | 悬空会话路由致「Not found」白屏 → 路由恢复前校验会话存在 | bug | A | verified | **S21 verified(2026-07-06,真机批 vNext-2)**:方案② main 预清两级全做(challenge 裁决:只修格式级=placebo)——tier-1 格式级同步清洗(S17 形态 B 毒源;`worktree "/"` shape-only 放行)+ tier-2 存在性查证(形态 A)+ store-get gate(A1 window-first 不回退)+ `[req014-preclean]` 留痕;`main/tabs-preclean.ts` 16 单测。**真机 verified**:ship1 冷启动剔形态 B + 旧格式 recent(截图 b2-coldboot-poisoned-ok);**tier-2 真机当场证伪 list 版**(`session.list?limit` 不生效→分页未尽 fail-open 恒空转=placebo)→ **改按 id `GET /api/session/{id}` 直查(PR #117)**,ship2 冷启动剔形态 A 毒键 + 顺带剔一真实悬空 tab(`danglingA 1→0`,截图 b2-ship2-tier2-ok);证据 [audits/s21](audits/2026-07-06-s21-realmachine-vnext2/verify.md);实现记录见档。**复现达成(2026-07-05,S17 T4)**:变体形态 B——旧格式 `tabs.recent`(无 dir 段)→ route.dir=undefined → 上游 titlebar 崩 → **整屏** ErrorPage 循环;整屏态 alpha 子组件全不挂 → 方案①守卫无效、**方案② main 预清实证可达**(删 global store 毒键即愈);证据+建议 [audits/s17-t4 §2](audits/2026-07-05-s17-t4-c28/verify.md)、档内复现记录;修法拍板就绪。历史:REQ-002 联调 BP-3;`tabs.recent` 指向已删会话 → 冷启动整屏 Not found 无恢复入口;**(/loop 2026-07-04 调查·deferred)**:原设「alpha 恢复层」杠杆不存在——恢复由上游冻结 `tabs.tsx` 主理;修法 renderer 守卫①vs main 预清 store② 需拍板,取决于 Not found 整屏/布局内(须真机复现)+ ② 触碰 base64 路由编码耦合(ADR-008);→ 并入 [[REQ-016]] 真机复现后定夺;详见 [requirements/REQ-014](requirements/REQ-014-dangling-session-blank-screen.md) 调查记录 |
...
| REQ-024 | 自动化 A2 增强:standard 可写档 + LLM 辅助解析 + 连败熔断 + 立即运行 + 预算/历史 UI | feature | A | verified | **S18 T8 shipped(2026-07-06,PR #106)**:①standard 可写档(alpha-automation-standard agent 注入:edit allow + bash 破坏类模式 deny(黑名单诚实非穷尽)+ 无人值守零 ask;启用确认 + 档位说明;存储层白名单放开)②LLM 辅助解析(规则失败后用户显式触发;临时会话一次抽取即删;产物仍过存储硬校验)③连败熔断(shouldTripBreaker 纯函数单测:3 连 failed/timeout(skip 不算尝试)→ 自动停用+disabledReason+通知;re-enable 清除)④立即运行(不改 next-fire;占并发位;计日 cap)⑤预算 UI 既有 + 历史 30 条 cap 既有(run 目录不自动删,留档说明);审计修复随 PR #109(bash 黑名单加固/LLM 临时会话删除重试);真机 E2E→真机批;详见 [requirements/REQ-024](requirements/REQ-024-automations-a2-enhancements.md)  **verified(2026-07-06,S27 场次二)**:standard 可写档启用确认(诚实标注黑名单非穷尽)→ 立即运行真会话:edit 创建 req024-proof.txt 通过 + `rm -rf` 被权限档 deny(文件仍在,report 自述「rm * 在 deny 列表」)+ 8s ok 终态=零 ask;证据 [req024-run-report.md](audits/2026-07-06-realmachine-vnext3/req024-run-report.md)。 |
...
| REQ-026 | 面向非技术用户的规范文档(D3 下沉拍板分期第一步:安装→登录→对话 + 常见错误 FAQ) | docs | X | shipped | **S18 T11 shipped+prod(2026-07-05,C alpha-web #6 已部署)**:`alphacodeone.com/getting-started`(装-登-用三步零术语 + FAQ×10 与 B11 口径一致 + 进阶只做索引;导航入口);落点=alpha-web(开批拍板);app 内帮助菜单链接随后续 A 侧小改;prod 200 + 内容实测;中文先行,英文随 C20 |
| REQ-028 | composer 真只读档:引擎 plan agent 通道 + 切换 UX | feature | A | shipped | **S18 T7 shipped(2026-07-06,PR #105)**:alpha-readonly agent config 注入(edit/bash 静态 deny,question/task 允许=交互差异;裸引擎证 deny 规则入引擎)+ PermChip 三档(cycle 判停,失败诚实回退不装成功)+ agentLabel 观察源一致性 + showCustomAgents 一次性 seed + 治理保护名单(X2);像素/会话实拍→真机批;详见 [audits/s18-t7](audits/2026-07-05-s18-t7-req028/verify.md) |
| REQ-029 | composer effort 接入 model variants:逐模型参数档 + B 侧透传核实 + chip 驱动 | feature | X | shipped | **S18 T9 shipped(2026-07-06,PR #107)**:①B 透传核实(OpenAI-wire spread 全透传✓;anthropic-wire 不映射 thinking → 支持面如实缩小)②variants 配置驱动(alpha-models.json:claude-opus/sonnet=OR 统一 reasoning{effort}、gpt-5.4-mini=reasoning_effort;direct/deepseek/nano 诚实不定义)③**echo 实验实锤**:variant 选中 → wire body 实际携带 reasoning_effort/reasoning(非 UI 自嗨,验收①机制面)④EffortChip 接真:驱动上游 model.variant.cycle+文本判停,控件缺失=模型不支持 → 禁用显示「—」(C28),移除「暂未接入」;BYOK/代理真机两路实测→真机批;详见 [requirements/REQ-029](requirements/REQ-029-effort-model-variants.md) |
...
| C16 | 卸载残留 ≈0.8GB 含凭证:清理方案 + app 内数据清除入口 | debt | A | verified | **S23 shipped(2026-07-06,PR #120)**:清除引擎 `data-clear.ts`(electron-free,13 单测:分级清单/守卫根 realpath 复核/symlink 不跟随/桥链只摘自有/TOCTOU 拒删/失败不静默)+「数据 ▸ 清除数据…」分级对话框(凭证级=清密钥+登出+respawn 防复活;全部级=仅打包态,先备份提示→红色终确认+引擎数据共享面 checkbox→停引擎→清→退出)+ `docs/UNINSTALL.md`(与引擎清单同源,菜单直达);钥匙串 safeStorage 项无 API 可代删=文档手动法(验收①「钥匙串项」按此收窄);dmg 内嵌指引未做(随下次真实发版评估);**verified 待真机**(对话框实拍/凭证级登出态/全部级 du 复核)→ 下一真机批;详见 [requirements/C16](requirements/C16-uninstall-residue.md)  **verified(2026-07-06,S27 场次二)**:凭证级真执行——UI 登出(侧栏变「登录」)+ byok/secret-files/mcp-secrets/engine-auth 日志 outcome=ok + respawn 防复活(alpha-secrets 空目录、无密钥复活;alpha-auth 44B 为 logout 登出态占位非有效 token)。全部级三段安全带对话框正确渲染(级别选择/备份提示/红色终确认列 477MB 体积+桥链 2 条+引擎数据 checkbox+「不碰项目/配置/自建」明示);删除边界守住(仅清 userData/~/.alpha/引擎数据,**项目文件·各项目 .alpha·~/.config/opencode·仓库 全部未触**)。**⚠️ 事故**:全部级经 AppleScript 自动驱动对话框链时被误执行(用户原意只验安全带不真抹)→ 抹掉本机登录/扩展/引擎会话库+B14 备份(不可恢复);经过与安全建议登记 [[REQ-050]]。 |
...
| REQ-044 | 迁移候选名字匹配把用户自建技能列为候选(替换风险)+ catalog mcp-builder 打包资产缺失(条目安装恒失败) | bug | A | verified | **S22 shipped(2026-07-06,PR #119)**:① provenance 终审下沉 main(`verifyLegacyProvenance`,electron-free)——skill=旧位与打包资产**逐字节比对**(pre-T2 安装是资产逐字节拷贝,漂移=用户内容→排除);mcp=catalog 形状(runner+包基名去钉版+参数数+无外来键+env⊆requiredEnvVars);plugin=基名等且未钉版∨同钉版;不可证明一律排除(fail-closed,宁漏迁不碰用户内容)+ `[req044-provenance]` main.log 留痕;IPC `ext-migrate-verify` + hub 按裁决过滤;+7 单测(455 全绿)。② catalog 撤 mcp-builder/canvas-design/brand-guidelines 三无资产条目 + 空壳 bundle:design 整撤 + bundle:dev 摘成员;补货正道=远程 catalog(REQ-032)零发版。**S23 补(2026-07-06)**:② 只撤了 A 内置半边——C 侧 catalog-src 仍在下发同三条(远端整份替换内置 → 联网用户撤架无效),经 alpha-web PR #7 补齐;根因登记 [[REQ-046]]。**verified(2026-07-06,S27 真机批 M1-4)**:隔离根开门演练——手写同名 `mcp-builder` 在场:迁移条「发现 1 项」**不列它** + main.log `[req044-provenance] excluded` 留痕 + 原文完好未触碰;字节拷贝 `safe-refactor` 正常迁移四要件全过([audits/vnext3](audits/2026-07-06-realmachine-vnext3/verify.md))。原始发现:S21 真机批 M1,证据 [audits/s21](audits/2026-07-06-s21-realmachine-vnext2/verify.md) |

## Active — P3(卫生)

| ID | 标题 | 类 | 仓 | 状态 | 备注 |
|---|---|---|---|---|---|
| REQ-048 | catalog 存量条目缺 per-entry version:每次发版全量误亮「可更新」角标(placebo 更新提示) | debt | C | verified | **S28 shipped+prod(2026-07-06,alpha-web PR #12)**:23 条补显式 `version:"1.0.0"` + 顶层 .3→.4 + **build-catalog.mjs 缺 version 从回填+warn 升级为拒绝构建**(红绿演练 PASS);prod 复验=验签 VALID + 28 条全显式 + 资产 sha256 抽样 MATCH。勘探修正原判:存量 receipts 记的是**日期版**(build 曾以顶层回填)→ 补 1.0.0 后旧 receipts 角标熄灭(误亮修复)但对未来 1.0.x 真更新**永久不亮**(数字段 2026>1 恒成立;装机面≈开发机,重装该条目即自愈,记档接受);A 侧 `versionLess`/`entryVersion` 零单测锁 = 残项(低优先,随下次 A 侧 hub 触碰补);**verified 待**S27 场次二顺带(hub 已安装 tab 角标归零 + 开发机条目重装收敛 receipts)。原发现:S27 真机批([audits/vnext3](audits/2026-07-06-realmachine-vnext3/verify.md));契约 [sprints/s28](sprints/2026-07-06-s28-prelaunch-fastlane/sprint.md)  **verified(2026-07-06,S27 场次二)**:快照 .4 装机 + 联网 .4 下发后 hub「已安装」tab **零角标**、无「有更新」分组;场次一误亮四条全熄。旧 receipts 熄灯残留(重装收敛)+ A 侧 versionLess 零单测锁 = 已记残项。 |
| REQ-045 | 撤下条目远程补货:mcp-builder/canvas-design/brand-guidelines 三 skill 资产经远程 catalog 上架(来源核验+NOTICE+sha256) | feature | C | verified | **S26 shipped(2026-07-06,alpha-web PR #10,已部署 prod)**:三条资产再分发自 anthropics/skills@`9d2f1ae`(LICENSE.txt 逐条核验 Apache-2.0;NOTICE.md 溯源随资产;canvas-design 字体 SIL OFL 1.1 许可随字体)→ `build-catalog.mjs` 发布(catalog 2026-07-06.2,98 文件 sha256 钉死 + ed25519 签名);prod 复验 = 验签 VALID + 抽样下载 sha256 全匹配;`bundle:design` 成员齐回归 + `bundle:dev` 补回 mcp-builder(验收④);**A 仓零动作**(REQ-046 单侧作者纪律首次真实演练);**verified(2026-07-06,S27 真机批 M1-1/M1-2)**:签名包 hub 三条目远程下发可见 → 安装四要件(真源+整目录桥+receipt origin:catalog v1.0.0+NOTICE 随资产)→ bundle:design 一键装远程成员扇出首例 84+3 文件全落([audits/vnext3](audits/2026-07-06-realmachine-vnext3/verify.md);首装曾踩 [[REQ-047]] 环境毒化,消毒后全过);契约 [sprints/s26](sprints/2026-07-06-s26-req045-restock/sprint.md);详见 [requirements/REQ-045](requirements/REQ-045-remote-catalog-restock.md) |
...
| D10 | ui-mac package.json license/author 补全 | docs | A | shipped | **PR #35**(→ [s9b](sprints/2026-07-03-s9b-hygiene/sprint.md));package.json 补 license:MIT/author/repository(jinjunnn/alpha-code),gates 绿;**陈旧注释子项已修(S19,2026-07-06)**:index.ts 版本注释去版本化改写(不再随升级过期)——D10 验收①②③全达成 |
...
| E6 | 数据库 MCP(sqlite/postgres 读 schema + SELECT) | feature | A | verified | **S23 shipped(2026-07-06,PR #120 + alpha-web PR #7 双侧上架)**:`@bytebase/dbhub@0.12.0 --readonly`(Bytebase 官方,MIT;PG/MySQL/MariaDB/SQLServer/SQLite;DSN 经 env {file:} 采集不落明文);钉 0.12.0=最后支持 --readonly CLI 档版本(0.13.0 起只读需 TOML,安装链无 TOML 通道,理由入 _verify);+ catalog 完整性回归锁单测(id 唯一/bundle 零悬空/钉版本/REQ-044 撤下条目不回流/--readonly 在场);**verified 待真机**(安装+SELECT/写拒绝走查) **verified(2026-07-06,S27 场次二)**:对 hub 同款 `@bytebase/dbhub@0.12.0 --readonly` 跑 MCP stdio(DSN→临时 sqlite):SELECT 通;INSERT/UPDATE 被拒(`READONLY_VIOLATION`,白名单 select/with/explain/analyze/pragma)+ 写后 count 不变——决定性解 `_verify` readonly 语义悬项。 |
