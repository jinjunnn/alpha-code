...
| REQ-044 | 迁移候选名字匹配把用户自建技能列为候选(替换风险)+ catalog mcp-builder 打包资产缺失(条目安装恒失败) | bug | A | registered | S21 真机批 M1 发现(2026-07-06):① 迁移候选 = 旧 XDG 根 skill 名 ∩ catalog 名(`extension-hub.tsx:scanMigration`)——用户自建 `mcp-builder`(与 catalog 同名)会被列为迁移候选,「迁移」调 catalog installer 重装 = **用 catalog 版覆盖用户自建版**(ADR-019 §4「只迁 alpha 自装」意图未落实,名字匹配不足以判 provenance);真实根迁移因此留用户批。② catalog `skill:mcp-builder`(builtinAssetKey `skills/mcp-builder`)资产**未随 app 打包**(Resources/skills 仅 skill-creator/alpha-upstream-sync/safe-refactor)→ 安装恒诚实失败。修向:迁移候选加 provenance 判据(仅账本 receipt 记录的 alpha 安装物)或迁移前 diff 提示;补打包 mcp-builder 资产或从 catalog 撤该条目。证据 [audits/s21](audits/2026-07-06-s21-realmachine-vnext2/verify.md) |
