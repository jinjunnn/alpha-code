...
> 下一个新需求编号:**REQ-044**(新需求一律 REQ-NNN;A/B/C/D/E 为历史审计系列保留原号,用户 2026-07-03 确认)。
...
| REQ-042 | REQ-040 丢弃陈旧默认服务器:静默无日志 + 陈旧键永不清理 | debt | A | shipped | S20 审计发现(2026-07-06):`getDefaultServer` 命中易失本地 URL 直接返 null,**零日志**(B11 反静默);陈旧 `defaultServerUrl` 键从不清除。**shipped(同日,S20 审计续批)**:治理下沉 main `getDefaultServerUrl`(读取的唯一消费点)——命中即 `main.log` 留痕 + 删键;谓词移 `shared/ephemeral-server-url.ts`(main/renderer 共用,connections re-export 保住既有面);renderer 检查降为纵深兜底(兜到 console.warn 不静默)。set-default 写入时不拦(彼时 URL 真活),只在冷启动读取处治理。**verified 待**下一真机批冷启动顺带看 main.log 行 + store 键消失 |
| REQ-043 | variant/agent cycle 的 DOM 轮询竞态:observer 滞后 >90ms 假报「切换失败」 | debt | A | shipped | S20 审计发现(2026-07-06):`switchVariantTo`/`switchAgentTo` 逐步 trigger + 固定 90ms 后读 DOM 判停;S20 verify 实测到一次 observer 滞后。**shipped(同日,S20 审计续批)**:新增 `cycle-to.ts` 等待原语(read/step 注入、不引 Solid)——单步「轮询等真实文本变化」判档(变化即返,典型切换更快),单步超时 600ms 无变化=控件无响应诚实 false,转满一圈判定不变;两个 switch 重接,7 单测含滞后 60ms 竞态回归例。**verified 待**真机 popover 点选实拍(REQ-041 既有残单同场) |
