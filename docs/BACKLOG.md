...
| REQ-042 | REQ-040 丢弃陈旧默认服务器:静默无日志 + 陈旧键永不清理 | debt | A | registered | S20 审计发现(2026-07-06):`getDefaultServer` 命中易失本地 URL 直接返 null,**零日志**(与 B11 反静默失败精神不合——若用户真经上游弹窗把固定端口本地 server 设默认,会每次开机被无声吃掉);`opencode.settings` 的陈旧 `defaultServerUrl` 键只被忽略、从不清除。修=命中时 log 一行(main 或 renderer)+ 顺手删键;快车道,下一真机批顺带 |
| REQ-043 | variant/agent cycle 的 DOM 轮询竞态:observer 滞后 >90ms 假报「切换失败」 | debt | A | registered | S20 审计发现(2026-07-06):`switchVariantTo`/`switchAgentTo` 逐步 trigger + 固定 90ms 后读 DOM 判停;S20 verify 已实测到一次 observer 异步滞后(F-2 重打包批)。滞后时 `cur===start` 早退假报失败,极端可越档(显示随 observer 自愈,但 toast 误导)。REQ-029/S18 既有模式,非 S20 引入;修=等待条件改「文本变化或超时」而非固定延时,或失败前重试一轮 |
