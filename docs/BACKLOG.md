...
| ID | 标题 | 类 | 仓 | 状态 | 备注 |
|---|---|---|---|---|---|
| REQ-072 | 斜杠命令菜单可用性重修:键盘导航失效/技能列表 12 条硬帽/搜索只匹配名字 三根因修复 + 类型 icon/来源标注/中文名/分组 展示重设计(Codex 参照稿对标) | ux | A | in-sprint(S33) | **用户真机报障(2026-07-09,2 截图)**:三根因登记时已源码钉死(`composer-autocomplete.tsx`)——①↑↓ 失效 = active 重置竞态(`:270` 每键 bump → `:214` items 变即 setActive(0),实现在而被自己覆盖)②技能不全 = `slice(0,12)` 硬帽(`:204`,引擎序非字母序)③搜索仅 `name.includes`(`:182`,描述不参与、无前缀优先、无结果菜单闪没);分期 T1 行为修复 / T2 展示重设计(类型 icon+来源收敛「内置/个人/项目/MCP」+出厂/catalog 补中文名、外来不机译);**3 拍板点同日全清(来源四档/分组分节/中文名供给)→ ready**;验收真源 [requirements/REQ-072](requirements/REQ-072-slash-menu-usability.md) |
