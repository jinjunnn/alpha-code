...
> 下一个新需求编号:**REQ-067**(新需求一律 REQ-NNN;A/B/C/D/E 为历史审计系列保留原号,用户 2026-07-03 确认)。
...
| B21 | BYOK 改键/删键即时生效(触发重注 env/respawn) | bug | A | shipped | **PR #48**;根因=env 桥 set-if-unset 滞留旧 key;修=自有注入权威覆盖/清除(用户值永不动,纯逻辑 5 单测)+ 改键回调触发重注+respawn;删键即时吊销;**verified(2026-07-07,装机 v0.1.2+PR#141/142 真机,用户真 key 在场)**:改键 4ms→respawn 2.5s→密钥文件即换新 key(判定面,旧 key 仍有效故不能靠聊天成功证明)→真消息 4.6s 回复;删键 3ms 即吊销(文件删+picker 即时落需KEY行+引擎 loud 500 非静默);复键走真实 UI 表单(需KEY行→测试连接✓316ms→保存)全链通;证据 [audits/2026-07-07-b21](audits/2026-07-07-b21-byok-realkey/verify.md);⚠️ 场中发现 [[REQ-061]](已配置行改键 UI 入口被弹层竞态挡死,IPC 语义不受影响);详见 [requirements/B21](requirements/B21-byok-key-live-reload.md) |
