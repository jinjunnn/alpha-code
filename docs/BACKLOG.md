...
| REQ-061 | 统一 composer 弹层 click-outside 竞态:点击会同步卸载被点按钮的交互(AddProvider 预设行/表单「返回」)→ 整层弹窗误关,已配置供应商改键表单入口不可达 | bug | A | shipped(PR #150) | **B21 真机批场中发现(2026-07-07,装机 v0.1.2+PR#141,3 次复现,证据 [audits/2026-07-07-b21](audits/2026-07-07-b21-byok-realkey/verify.md) §REQ-061;编号让位——原登记 REQ-060 与并行 session 的「项目级扩展 .alpha-only」撞车,本 bug 改 REQ-061)**:根因=`alpha-composer.tsx useChip.onDoc`(document click)以 `e.target.closest(".a-pop")` 判内外,Solid 委托事件下点击处理器同步重渲染把被点按钮 detach → closest=null → 误判外部点击关层;`.a-pop` 的 stopPropagation 拦不住同节点 listener。影响面:①step1 预设行→表单(改键主路径,硬断)②表单「返回」③「其他/自定义端点」(推定);**需KEY行/添加行/测试连接/保存不受影响**(点击不卸载自身)。修法:onDoc 改 `e.composedPath()` 包含判定(dispatch 时快照,detach 免疫);快车道,修后下一真机批补「已配置行改键」UI 走查;详见 [requirements/REQ-061](requirements/REQ-061-composer-popover-click-race.md) |
...
| REQ-062 | 去 opencode 化·路线A:系统提示词品牌转写(ext `experimental.chat.system.transform` 运行时转写底座自指)+ identity 删 "built on opencode" + `/init` `/review` 同名覆盖换 alpha 模板 + customize-alpha skill 接替 + **general/explore 子 agent 同名 prompt 重写**(2026-07-08 用户追加)+ lsp.txt 工具描述转写(tool.definition) | feature | A | shipped(PR #152) | **用户拍板(2026-07-08,GOALS G6 / [[ADR-015]] 修订)**:源码钉死——8 个底座 .txt 首行全自称 OpenCode(anthropic.txt:1),instructions 纯叠加压不掉(request.ts:58-66 底座在前同条 system);transform hook 是唯一触及底座+environment 的零-fork 接缝(plugin/index.ts:291-296)。NON_GOALS#4 风险标注:experimental hook,逃生 `ALPHA_PROMPT_REBRAND_DISABLE`,失效最坏=品牌未转写(外观级);sync tripwire(REQ-012 已接线)覆盖底座变更复核;详见 [requirements/REQ-062](requirements/REQ-062-prompt-rebrand-route-a.md) |
| REQ-063 | 外部生态继承 default-deny + 打开项目 consent 导入门(`.claude`/`.agents` skills + CLAUDE.md;[[ADR-024]] 执行载体) | security | A | shipped(PR #153) | **用户拍板(2026-07-08)**:sidecar 默认注入 `OPENCODE_DISABLE_EXTERNAL_SKILLS=1` + `OPENCODE_DISABLE_CLAUDE_CODE_PROMPT=1`(set-if-unset;逃生 `ALPHA_ECOSYSTEM_INHERIT=1` 整机恢复上游行为);打开项目检测外来内容 → 信任门 sheet(REQ-060 同款)→ 同意 = 安装期转换导入项目 `.alpha`(ADR-023,receipts origin imported-*),非重开继承;**硬前置/发布闸 = 全局存量一次性迁移门**(~/.claude/skills 等非空首启必弹,防「技能丢了」重演——作者本机 graphify 即存量);consent 载体 = 原生 sheet(主)+ **系统级导入 skill**(对话式补充/重导入通道,2026-07-08 用户补充);呈现半边(来源标注)= [[REQ-066]];详见 [requirements/REQ-063](requirements/REQ-063-ecosystem-inheritance-default-deny.md) |
| REQ-065 | `.alpha` 纯度反向收口:出厂/系统件退出 `~/.alpha/skills`(skills.paths 直指 app 资源),`.alpha` 只承载用户自有内容 | debt | A | shipped(PR #151) | **用户点名(2026-07-08)**:skill-creator/agent-creator 出厂链落 `~/.alpha/skills` = REQ-052 两跳桥遗留;裁定非 bug——REQ-059 零 `.opencode` 后「先落 .alpha 中转」对出厂件的理由消失(ADR-019 2026-07-08 修订);修 = skills.paths 直指 Resources + reconcile 拆存量我方链(用户自装不碰);验收核心 = `~/.alpha` 全树 100% 可溯用户动作;详见 [requirements/REQ-065](requirements/REQ-065-alpha-dir-user-only.md) |
| REQ-067 | 出厂治理内置化:上游默认禁项(customize-opencode)零明文 —— 出厂即禁(内存注入),`alpha.jsonc` 不再出现 permission.skill deny 与占位 command;治理面可解禁(allowFactory) | debt | A | in-sprint(S31) | **用户拍板(2026-07-08,REQ-065 修订同源口径)**:alpha 原生的封装起来,只有用户自己的 agent/skill/command/mcp 才进配置文件(对齐 Claude Code settings.json 心智);出厂路径半边已随 REQ-065 修订(PR #155)内存化,本项收 deny 半边:main 算 effective(出厂清单 − 用户解禁)→ env → ext config hook 注入;存量明文 boot 剥离;菜单过滤读 effective |
| REQ-066 | 斜杠菜单卫生:治理禁用项不显示(customize-opencode 禁了仍在菜单)+ 命令来源标注(内置/技能/项目/MCP/导入) | ux | A | shipped(PR #149) | **用户截图实证(2026-07-08)**:上游 deny 后 command.list 仍返回条目(governance.ts:14 已知泄漏),REQ-037 时代只能占位缓解;**REQ-055 后菜单已是 alpha 自有组件 → 过滤权在我方**——按治理状态过滤(不靠文案前缀),引擎占位 template 保留作键入兜底;来源标注解「命令来路不明」(source 字段 + receipts + 内置名单交叉);详见 [requirements/REQ-066](requirements/REQ-066-slash-menu-governance-filter.md) |
