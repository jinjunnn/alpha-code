...
| REQ-019 | 定制中心 v3-M2:hub **横向 tab** IA(2026-07-04 拍板,否决左栏)+ 逐类型详情页(数据边界/实时依赖检测)+ 「添加」三档分流 + 更新通道 + 导入 folder/git | feature | A | in-sprint | **S13 headline(2026-07-04)** → [sprints/s13](sprints/2026-07-04-s13-ext-hub-m2/sprint.md);T1/T2 已 shipped(横向 IA+详情页,交互定稿 designs/2026-07-04-ext-hub-m2);依赖 REQ-018(shipped);吸收 E11;MCP tools 清单=catalog 元数据(引擎无查询路由,已核);详见 [requirements/REQ-019](requirements/REQ-019-ext-hub-detail-lifecycle.md) |
...
| REQ-023 | 扩展安装供给链:官方扩展全配置化 + 离线资产通道(vendored plugin/agent,绝对路径写 plugin[] 绕 npm 下载)+ 安装管线状态机 | feature | A | in-sprint | **用户 2026-07-04 提出(配置化/下载源/cache/pipeline),追加进 S13(T9)**;skill 离线已有、MCP 已治理(A2),净新增=plugin/agent vendored 通道 + 状态机;不自建 CDN(→E10/REQ-020);详见 [requirements/REQ-023](requirements/REQ-023-ext-supply-chain.md) |
