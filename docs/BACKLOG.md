...
| ID | 标题 | 类 | 仓 | 状态 | 备注 |
|---|---|---|---|---|---|
| REQ-073 | 统一装配弹窗(@/+ 合一)+ 模式收编:重设计 @ 弹窗、移除 build 组件、计划模式收进弹窗以「⊗ 计划」chip 呈现(Codex 对标) | ux | A | registered | **用户直提(2026-07-09,5 截图)**:现状 @/+ 两弹窗割裂(+ 弹窗四条扩展行全是同一 setExtHubOpen 动作 = placebo 分身)、AgentChip 信息量近零;方案 = 一个组件两触发器(添加/Agent/文件/扩展 四节,REQ-072 底座全复用)+ 去 AgentChip(默认缺省参数=引擎 build)+ 计划模式 toggle→chip(agent:"plan",Shift+Tab,会话级)+ 权限联动/第三方主档动态项;**6 待拍板点**(GLOSSARY v2 口径/+ 触发实现/作用域/扩展节收敛/第三方主档/默认参数)→ 拍板后 T0 设计稿门;验收真源 [requirements/REQ-073](requirements/REQ-073-unified-assemble-popup.md) |
