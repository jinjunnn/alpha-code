...
> 下一个新需求编号:**REQ-050**(新需求一律 REQ-NNN;A/B/C/D/E 为历史审计系列保留原号,用户 2026-07-03 确认)。
...
| ID | 标题 | 类 | 仓 | 状态 | 备注 |
|---|---|---|---|---|---|
| REQ-047 | shell-env 探针把会话级隔离/调试 env 永久腌进缓存:安装物静默落死目录(placebo ok:true)+ ALPHA_CDP 被腌 = Finder 启动开调试端口 | bug | A | shipped | **S27 真机批发现(2026-07-06,P0)**:`shell-env-cache.ts` 探针 spawn 登录 shell **继承 app 自身 env** → 批测/终端启动时的 `ALPHA_GLOBAL_DIR`/`OPENCODE_CONFIG_DIR`/`ALPHA_OPENCODE_HOME`/`ALPHA_MIGRATE_ENABLE`/`ALPHA_CDP` 被 `env -0` 回吐并写进 `alpha-shell-env.json`,此后每次启动(含 Finder)0ms 套用;**自续毒化**:套用→process.env 带毒→异步再探测又继承→缓存永不自愈(本批确定性复现 ×2,证据 [audits/vnext3](audits/2026-07-06-realmachine-vnext3/verify.md) + `req047-poisoned-shell-env-cache.json`);真实用户可踩(终端 export 过 OPENCODE_CONFIG_DIR 再从终端启动一次即中毒);**shipped(同日,S27 快车道,PR #127)**:①探针净化 `minimalProbeEnv`(同步+异步两处 spawn 只带自举白名单+系统 PATH;缓存语义=「登录 shell 自己导出什么」,干净刷新对存量毒缓存整份重写自愈)②读侧 `sanitizeCachedShellEnv` 剥离 8 控制键 + `[req047]` loud 留痕(剥空→null 回退同步干净探测);+5 单测(500 全绿);真 export 场景不受影响(preferAppEnv 层照常);**verified 待**真机(毒缓存自愈 + Finder 启动零 9222)→ S27 下场次;本机已先行消毒(缓存删除 ×2) |
...
| ID | 标题 | 类 | 仓 | 状态 | 备注 |
|---|---|---|---|---|---|
| REQ-048 | catalog 存量条目缺 per-entry version:每次发版全量误亮「可更新」角标(placebo 更新提示) | debt | C | shipped | **S28 shipped+prod(2026-07-06,alpha-web PR #12)**:23 条补显式 `version:"1.0.0"` + 顶层 .3→.4 + **build-catalog.mjs 缺 version 从回填+warn 升级为拒绝构建**(红绿演练 PASS);prod 复验=验签 VALID + 28 条全显式 + 资产 sha256 抽样 MATCH。勘探修正原判:存量 receipts 记的是**日期版**(build 曾以顶层回填)→ 补 1.0.0 后旧 receipts 角标熄灭(误亮修复)但对未来 1.0.x 真更新**永久不亮**(数字段 2026>1 恒成立;装机面≈开发机,重装该条目即自愈,记档接受);A 侧 `versionLess`/`entryVersion` 零单测锁 = 残项(低优先,随下次 A 侧 hub 触碰补);**verified 待**S27 场次二顺带(hub 已安装 tab 角标归零 + 开发机条目重装收敛 receipts)。原发现:S27 真机批([audits/vnext3](audits/2026-07-06-realmachine-vnext3/verify.md));契约 [sprints/s28](sprints/2026-07-06-s28-prelaunch-fastlane/sprint.md) |
