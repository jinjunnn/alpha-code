...
| REQ-043 | variant/agent cycle 的 DOM 轮询竞态:observer 滞后 >90ms 假报「切换失败」 | debt | A | shipped | S20 审计发现(2026-07-06):`switchVariantTo`/`switchAgentTo` 逐步 trigger + 固定 90ms 后读 DOM 判停;S20 verify 实测到一次 observer 滞后。**shipped(同日,S20 审计续批,PR #115)**:新增 `cycle-to.ts` 等待原语(read/step 注入、不引 Solid)——单步「轮询等真实文本变化」判档(变化即返,典型切换更快),单步超时 600ms 无变化=控件无响应诚实 false,转满一圈判定不变;两个 switch 重接,7 单测含滞后 60ms 竞态回归例。**verified 待**真机 popover 点选实拍(REQ-041 既有残单同场) |
