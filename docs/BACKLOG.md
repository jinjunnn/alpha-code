...
| REQ-015 | 冻结前端 typecheck 偏斜:session-ui(546 后新增)依赖新版 ui API 与冻结 ui 不兼容 | debt | A | shipped | **S17 T2 方案5 落地(2026-07-05)**:push 门 rewire `.githooks`(=alpha-check,与 CI 1:1)+ husky prepare 重置自愈重挂(根因:每次 bun install 冲掉 hooksPath);方案1/2/4 结构性否决(session-ui 在 ADR-020 §3 红线)、方案3 被支配,ADR-020 修订在册;session-ui 全量 turbo 红 = 接受的已知偏斜;verified 待 hooked-push 实测回填;详见 [requirements/REQ-015](requirements/REQ-015-frozen-frontend-typecheck-skew.md) |
