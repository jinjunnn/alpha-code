...
### REQ-063 项目导入门 —— 全链 PASS(2026-07-08 三场,用户协助点「导入」)
- 测试项目 `/Users/tide/app/alpha-external-test`(`.claude/skills/haiku-answers` + 根 CLAUDE.md);
- 进入会话 → 原生弹窗出现 → 用户点「导入」→ **全链落盘**:技能复制 `.alpha/skills/haiku-answers/` +
  receipts `origin: imported-claude` + CLAUDE.md → AGENTS.md(带快照溯源头)+ prefs 记账
  `externalImport: {v1, imported}`(二次进入不再弹,机制 + 单测);
- **当场抓到实现缺口**:导入只复制文件、漏写项目 `alpha.jsonc` 的 skills.paths 注册 → 引擎看不见
  (`/haiku` 菜单为空)。当场修(PR #158,回归锁 2 例);修后 dispose → **`/haiku-answers[导入]`
  出现在斜杠菜单,徽标正确**。
- 场中连带用户报障 → [[REQ-068]]:「打开项目…」选完不切换、零反馈(上游/alpha 两套项目列表不通),
  同 PR 修复(真机切换路径因 ALPHA_OPEN_DIR 短路未走原生框,修复逻辑真机复验归下批)。
...
| REQ-063 | ④逃生开关(ALPHA_ECOSYSTEM_INHERIT 重启)⑤项目隔离(双项目对照) | 小残项(重启轮次),归下一真机批;主链全验(上) |
| REQ-066 | ②键入全名占位响应;MCP 徽标抽查(本机无 MCP prompt 命令) | 小残项,顺带下批 |
| REQ-068 | 修复后「点开项目→自动切换」的无短路真机走查 | 本场用 ALPHA_OPEN_DIR 短路驱动,原生框路径待用户日常使用自然验证 |
