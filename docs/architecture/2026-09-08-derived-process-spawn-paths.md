...
6. **落地了(2026-09-09,`#1321`)**:§8.2 的 19 行现在住在 `packages/ui-mac/src/main/process-fence-profile.ts`
   (唯一渲染点),§7.3 那套「从产物派生写盘调用点」的判据以源码 AST 的形态入仓并接进闸门
   (`scripts/process-fence-write-sites.tsv`);REQ-138 那层已拆。形状与判据地图见
   [`2026-09-09-req159-process-fence.md`](2026-09-09-req159-process-fence.md)。本文档仍是对照物,不改。
