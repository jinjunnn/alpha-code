...
3. 屏障不再有独立 deadline。旧实现在 1.5s 处把「还没收敛」抛成一次请求失败，交给
   1/2/4/8s 指数退避，而**退避窗口内一个探针都不发**。现在退出条件三个：marker 答了、
   探针给出**真实故障**（只有 `404` / `ProviderNotFoundError` 算「尚未收敛」，其余一律抛
   `ModelContractError` 交给既有恢复链），或调用方取消。屏障不再自己制造失败，但它也
   **不吞真实失败** —— 包括「引擎这一轮已经答了 503、而 `catalog.updated` 恰好同时到达」
   这一格：那时按**失败的来源**判（只有我们自己为重探发的取消不算故障），不按事件是否落地判。
...
6. 第 5 条有一个必须一起做的推论：**在途 `list()` 的拒绝要在发出处就有人接**。它从前挂在
   固定预算上，被丢下时既不 settle 也不 reject，所以「无主」是安全的；改挂到链的生命期
   之后，`supersede`/卸载会**当场**把它拒掉，而 `runModelChain` 在它与真消费者
   （`loadEngineModelsWithRetry`）之间有三个早退点（`currentRead` / `catalogRead` /
   `auth+keys`），退避循环自己也会在 `isStale()` 上丢下一条刚发出的 listing —— 每一处早退都
   留下一个**未处理拒绝**。它不改任何用户可观察的值，但在 renderer 里是真的
   `unhandledrejection`。实证：`#891` 合入后，`new-session-workspace.cases.ts` 里两条走
   「首页提交 → 会话页挂载 → 链未收敛就卸载」的生产路径用例整片变红，而两轮
   `expect() calls` 都是 101 —— **断言的值全对**，红的是拒绝被记到了用例头上。
   修在**发出处**，不在三个早退点各补一句：后者是按实例修，下一个早退点会再漏一次。
