...
- **非 darwin 未测**(与前四轮同)。
- ~~**并集裁剪规则(取前 K 的 K 是多少、按什么排序)本轮没有定**~~ —— **已由实现票 `#1321` 定下,见 §8。**
  本轮只给出了它必须存在的理由(§2.2)与量级参考(本机 N=5)。
...
## 8. 裁剪规则(实现票 `#1321` 定,2026-09-09)

落地在 `packages/ui-mac/src/main/process-fence-profile.ts` 的 `selectWorkspaceUnion` 与 `trimUntilCompiles`,
判据在 `process-fence-profile.test.ts`(顺序相等、K 封顶、排除、试编译丢尾各一格)。规则与理由:

| # | 规则 | 理由 |
| --- | --- | --- |
| 1 | `~/code-puppy` 恒在首位,不可丢 | §2.3 第一条:renderer 重载后的启动草稿恒落在它上面,不在集合里 = 每次重启后落地即不可写 |
| 2 | 其后按 `tabs.recent.key` 指向的目录 → `tabs` 数组顺序(draft tab)→ `tabs.info` 插入顺序(session tab) | `tabs.recent` 是 store 里唯一的「最近」信号(tabs.tsx:67),数组顺序 = tab 栏顺序;没有更细的 LRU,本票不造一个 |
...
丢到底仍失败为什么是「拒绝 fork」而不是「少放几个工作区」:到那一步并集只剩 1 条,失败原因不可能是并集大小,
而是 profile 语法 / 根路径异常;放行会是「前提为假的闸门」。§2.2 说的「fail-closed 到少放几个工作区」在第 7 条
的循环里已经做完,这是它的最后一档。
