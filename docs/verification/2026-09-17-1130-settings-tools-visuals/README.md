...
6. 三态 radiogroup 的方向键按本仓唯一的键盘契约(`alpha-ui/roving-focus.ts`,C21 AC2 棘轮)实现:
   →↓ / ←↑ 在组内环形移动**并激活**(APG Radio Group 的「移动即选中」),Tab 进出整组一次;
   帧外说明写的「左右键切换、空格确认」在这套契约下是「左右键即切换即生效」——
   收紧一点即存不变,class / service 层的「启用」照样先弹确认条。
