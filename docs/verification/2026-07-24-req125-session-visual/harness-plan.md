...
行 ID 均指同目录 `matrix.md`。2026-08-06 收口采用 production-component headless harness:
只允许 loopback Vite + `/Applications/Google Chrome.app` 的真 `--headless=new` 进程;
不得启动 Electron、Alpha Code、前台 Chrome 窗口、deep-link 或 packaged app,不得读取账号、
API key 或 owner 桌面状态。macOS 的 `open -g`/background activation 不等于视觉静默,不作为
本环境替代方案。

## 收口复跑(无界面,现行方法)

基点与每帧 hash 由 `harness/capture-metadata.json` 记录。以下步骤只在本仓 worktree 和
loopback `127.0.0.1` 内运行:

```sh
bun install --frozen-lockfile --ignore-scripts
packages/ui-mac/node_modules/.bin/vite build \
  --config docs/verification/2026-07-24-req125-session-visual/harness/vite.config.ts \
  --outDir /private/tmp/req125-harness-dist --emptyOutDir
python3 -m http.server 4173 --bind 127.0.0.1 \
  --directory /private/tmp/req125-harness-dist
node docs/verification/2026-07-24-req125-session-visual/harness/capture.mjs
```

capture 完成后必须停止 http server,并确认 4173 无 listener、没有匹配本 harness profile 的
Chrome 进程。驱动固定 1440×900,逐态同时设置 `data-color-scheme` 与
`prefers-color-scheme`,输出 `*-headless.png` 和 sha256 元数据。

下面「真机部分」保留为批1–3 的历史复现说明;owner 活跃使用桌面时不得执行。
...
- fixture 与真机 seed 用同一套演示值(文件名/行号/计数对齐设计稿帧内演示数据),
  保证与整页帧可互检。
- harness 文件落本目录 `harness/`;批4 由 `capture.mjs` 驱动 production-component
  harness 在真 headless Chrome 中采集,不创建前台窗口。
...
| J4/J7   | #558/PR#571 已确认无生产数据面 → N/A,禁止合成虚假态                                                                                           |
| J5/J6   | production dock 确定性态 → 截挂载区,建立回归基线                                                                                              |

## 产物归档

截图命名 `<行ID>-<light|dark>-headless.png` 落 `shots/`;批1–2 历史证据保留原命名。
采集完成后更新本目录 README(判定汇总:每行 PASS/FAIL/N/A + FAIL 的 bug 票号),
矩阵回填采集状态列。
