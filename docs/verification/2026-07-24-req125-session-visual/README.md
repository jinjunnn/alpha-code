...
- production Solid 时间线与现役 CSS 经 loopback-only Vite 装配,Google Chrome
  ...
  零前台窗口。
- 居中胶囊按已批稿补齐压缩图标、「上下文已压缩 · 保留要点」和 chevron。控件使用
  原生 `button`/`aria-expanded`,真 Chrome 中 Enter、Space 与鼠标均完成展开/收起;
  展开体只复用完成态 `assistant.summary=true` 的引擎摘要,不生成、猜测或暴露半截摘要。
- 证据:[浅色](shots/H4-light-harness.png)
  `906d35dfbfc024a41957141b58148058659100e657f2b1790326b3928cf4bb33`;
  [暗色](shots/H4-dark-harness.png)
  `77a589228932485398e7cb413cb084f2a9d107ee16c139e701927a67950c6648`。

## 2026-08-08 F2 #863 复验

- production Solid 时间线与现役 CSS 经 loopback-only Vite 装配,Google Chrome
  `151.0.7922.77` 以 `--headless=new` 重采明暗两帧;零 Electron、零账号/API key、
  零前台窗口。
- 折叠卡头在两主题均显示「思考 6 秒 · 规划探查顺序」,只在完成态投影起始行的
  显式标题并保持正文折叠;缺少起始标题时仅保留时长、不回溯或暴露普通推理正文,
  流式期不显示摘要,由组件测试钉住稳定降级。
- 证据:[浅色](shots/F2-light-harness.png)
  `f3a0976ba75e6e35afee411b7e208a812529dc774a897d8c5ce650d3b203cccd`;
  [暗色](shots/F2-dark-harness.png)
  `367de41b47219ca4a0c850fae4fda061551015398be759eb1331f0fcc1e38028`。
