...
## 任务拆分(按优先级序)
1. **T1 IA 重构**:hub 内左侧竖栏(浏览:推荐/连接器/技能/Agent/插件/套件;管理:已安装(n)/有更新(n);构建:创建/导入;云:云能力[随 REQ-020 点亮]);全局搜索持久(不随分区清空)。
2. **T2 详情页框架**:点卡片 → hub 内二级页(返回键 + Esc 逐级);通用头部(图标/来源/许可证/版本/verified;`_verify` 未核项显式标「待核实」)+ 通用区块(简介/类型专属/数据边界/运行时依赖/所需密钥/操作区)。
