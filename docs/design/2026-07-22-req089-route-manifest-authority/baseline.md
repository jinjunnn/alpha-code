...
- **F8 —— 模板字符串里的假 import 能吞掉后面那句真 import。** 语句正则从**任意行首**的
  ...
  **↑ 这句已被 R9 证伪,见 §5.8:失配也可能少抹,少抹会把这条假绿原样放回来。**
...
### §5.8 第九轮对抗审计(R9)与本轮收口(2026-07-26)

R9 审 §5.7 的字符串抹白时,**证伪了那条方向性声明**。「残余失配只会多抹 → 丢跨度 → mention 归
`aliased` → 红」是错的:失配**也会少抹**,而少抹会把 R8 那条假绿原样放回来。三条构造都经 Bun TSX
转译确认语法合法,在当前判据下都产出 `[imported, applied]`(诚实读法应为 `[aliased, applied]`):

1. 正则字面量里的引号 —— `/["]/` —— 加一段 `\` 续行的字符串;
2. JSX 文本里的裸引号 —— `<p>6" of rope</p>` —— 加同样的续行;
3. `${…}` 里的正则 `/}/`(提前结束代换)加一个标签模板(把词法器再交还到字面量之外)。

机理是同一个:`withoutLiteralText()` 是**字符串**词法器,不是 JavaScript 词法器。它分不出正则字面量
与除法,也分不出 JSX 文本与代码 —— 这两件事都由它没有的语法决定。任一误判开出一个并不存在的字面量,
下一个真引号把它关掉,**再往后真字面量的文本就被当成代码交回来**,于是 R8 的假绿(字面量里伪造的
`import {` 开出跨度、吞掉后面那句真的默认导入,任意模块的默认导出遂读作未改名的 named specifier)
从抹白的另一侧原样回来。

**裁决:不再追。** 纯词法判据无法正确处理「正则 vs 除法」歧义与 JSX 文本,这不是再补一轮能封住的
缺口,而是**这条路的固有上限**。R6 时已确立「这条 ratchet 是**漂移信号**而非安全断言」的有界立场;
问题从来不是立场,而是实现与文档反复宣称了超出它能力的东西。所以本轮**一行扫描器逻辑都没改**,只做
宣称与能力的对齐:

- 删掉那句被证伪的方向性声明(代码注释与本文 §5.7 两处),代之以如实表述:**失配可能多抹也可能
  少抹;少抹会恢复假绿**。
- 三条构造写进文件头,作为「**已知、不可由词法手段封堵**」的边界说明 —— 不是待办。
- 同时加了三条**文档性测试**(`test.each`,名字以 `KNOWN HOLE —` 起头):它们**故意断言错误答案**
  `[imported, applied]`,并注明诚实读法是 `[aliased, applied]`;哪天其中一条变红,说明洞被封了,
  删用例并改文件头。成本低到值得加 —— 复用现成的 `factoryNameSites()`,零新增辅助函数。
- 把「**主判据是执行类测试**」提到文件头最显眼处:`route-deep-link-consumer.test.ts` 跑真实生产
  `createDeepLinkConsumer` 与真实 manifest 编解码,断言观测到的导航目标 == manifest 独立派生的
  href;**任何改变 app 实际行为的改动都会打红它**。ratchet 只覆盖 `layout.tsx` 那一小段执行测试
  够不到的胶水,且**只对无意漂移有效**。

数字(对照 base `4744aa5bb`):定向测试 39 → **42 pass / 0 fail**;`bun test src` 2979 →
**2982 pass / 0 fail**;`typecheck` 196 → **196**(全部来自 worktree 符号链接的 `packages/app`
路径别名假红,fail-set 与 base 完全一致,本文件 0 条);prettier `--check` 通过;oxlint 3 warnings /
0 errors,三条都落在未改动的既有 `!` 断言行,与 base 一致。
