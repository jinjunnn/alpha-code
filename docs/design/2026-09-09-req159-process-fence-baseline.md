...
| # | 未闭合 | 现状 |
|---|---|---|
| U1 | 原生模块在**签名包内**能否加载 | 地面真相已收窄(出货包已带三个 `.node`;`pty.node` 与 app 同 Team;app 未开 `cs.disable-library-validation` ⇒ 同 Team 签名才过)。**但从未写过、签过、在打包 app 里加载过任何新 addon。** |
| U2 | 多工作区 vs 可写集不能加宽 | 形状成立(取并集,两个工作区都能写、越界仍 0 落盘);只放行一个时第二个工作区**打得开、读得了、一写就 EPERM,且失败不在任何启动日志里**。**「打开新目录 ⇒ respawn」既没实现也没实测。** |
| U3 | 出货形态(node + 打包产物 + 围栏)未合成 | `sidecar.js` 顶层即 `getParentPort()`,脱离 `utilityProcess` 起不来 ⇒ **与 U1 互锁** |

U1/U2 各自成票,**先于**围栏实现票落地;U3 随 U1 闭合。
