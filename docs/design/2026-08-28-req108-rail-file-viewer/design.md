...
- **右栏内本地文件的预览面只有这一块**(#244 AC1 的「唯一 preview surface」):
  ...
  - 时间线写入/编辑文件卡 → 审查面板;审查文件卡头部新增「查看整份文件」动作
    ...
    文件)。**已裁决采纳(owner 2026-08-28)**,理由:对改动过的文件,diff 本身
    就是用户要看的东西;且该解释保住了 AC1 的「唯一右栏 preview surface」。
...
- **viewer 自带下载 / 打印:去掉(owner 裁决 2026-08-28,推翻本稿初版的
  「放行但收口」推荐)。** 能力边界经实测钉死后按裁决选定形态如下。

  **实测(2026-08-28,Electron 42.3.3 裸 spike,三臂纪律:已知坏文件
  `ERR_FILE_NOT_FOUND`/frames=0 先证明手段有效;判 UI 存废一律截图,不用 DOM 探针)**:

  | 手段 | 实测结果 |
  | --- | --- |
  | 无参数(基线) | 完整工具栏:页码、缩放、旋转、批注、**下载、打印**、更多;左侧缩略图栏 |
  | `#toolbar=0` | **整条工具栏 + 缩略图栏全部消失**,只剩正文;内容照常渲染(frames=1) |
  | `#navpanes=0` | 只藏缩略图栏,工具栏(含下载/打印)仍在 |
  | `#zoom=200` | 缩放钉在 200%,工具栏仍在 |
  | 按钮级定制 | **不存在**:工具栏是 Chromium PDF 扩展自己的 UI,只有整条全有/全无,没有「摘掉两个按钮保留其余」的手段 |
  | `before-input-event` 拦 Cmd+P | **有效**(事件捕获 + preventDefault,实测触发并拦下) |
  | 合成点击探按钮行为 | **本轮不可测**:`sendInputEvent` 鼠标事件进不了 viewer 的独立子 frame —— 正样本(点缩放 +)也无任何效果,故「真实点击下载按钮走不走 `will-download`」不下结论 |

  **选定形态(与裁决一致,且可实现)**:PDF 一律以 **`#toolbar=0`** 装载 ——
  下载/打印按钮**不存在**(整条工具栏移除),没有「按钮在但点了没反应」的诚实性
  问题。纵深两道保险:该 WebContentsView 上 `before-input-event` 拦 Cmd+P / Cmd+S,
  隔离 session 上 `will-download` 注册并 preventDefault 兜底(拦截即为异常,fail-closed;
  Electron 默认无右键菜单,不另开菜单即无第三条路)。

  **代价(如实)**:没有页码指示、缩放按钮与缩略图 —— 翻页靠滚动(本轮 spike 用的
  是单页样张,多页滚动未实测 —— 实现票的 VERIFY 须含一个多页样张的滚动翻页格),
  这是「全有/全无」边界下服从裁决的必然取舍。若将来需要缩放/跳页,只能由
  查看器自身 chrome 以 `#zoom=` / `#page=` 片段重载实现(片段参数实测逐个被认),
  本稿不承诺该扩展。

  **与查看器头部动作的关系**:头部「更多」菜单里的「另存副本」是应用自己的诚实
  动作(#244 AC6 明列,经系统保存对话框、目标用户亲选),保留;被去掉的是
  Chromium viewer 自带的下载/打印 UI 及其快捷键通路,两者不是同一件事。
...
## 10. 三个裁决点(owner 2026-08-28 已全部裁决)

1. §3 对 #244 AC1 的「一跳可达」解释 —— **采纳**(理由已记入 §3);
2. §5 PDF viewer 下载/打印 —— **去掉**(推翻初版「放行但收口」推荐;实测能力边界
   与选定形态见 §5);
3. §4 载体 = WebContentsView 叠放 —— **确认**(含遮挡合同与证据形态的代价)。
