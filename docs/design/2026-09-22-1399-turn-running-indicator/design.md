...
> **owner 2026-09-22 批准**:三个待定全按默认(「正在思考」胶囊并进脚行、带计时、不悬浮),并要求补画
> 第六态「模型反问你」—— 已并进等你面(§3、§4)。帧已并入活稿
> [`current/conversation-timeline/design.html`](../current/conversation-timeline/design.html) 的 `②`
> 节末,锚 `#turn-running`;台账
> [`current/conversation-timeline/components.md`](../current/conversation-timeline/components.md) 已回填;
> **本目录自此冻结**。帧见同目录 [`frame.html`](frame.html)(运行面四个子状态 / 等你面两种触发 /
> 三种结局;右上角切浅深两色、切正常 / 减弱动效)。
...
**运行面**(脉冲点 + 「正在生成」+ 计时)和**等你面**(静止暂停符号 + 主句 + 去向;等你批准与
等你回答共用,只换文案)。一轮从发出到结束,这一行始终在;结束的同一帧让位给回合的结局
(脚注 / 中断行 / 错误卡 / 空回合行)。

**取代**:活稿 `②` 里的「正在思考」胶囊(`.thinking`,实现为 `ThinkingRow`)。它今天只在
首个 part 到达前存在,是这一轮的**第一格**;本稿把这一格并进脚行 —— 同一位置、同一时刻,
换成脚行的运行面。owner 2026-09-22 批准并入;活稿里该胶囊帧保留并就地标注。
...
| 模型反问:`question` 工具的提问卡**停靠在输入框上方**,不在时间线里;「提问 / 提交回答 / 暂不回答」 | `session-composer-dock.tsx:5, 120-123, 197`(`headPendingQuestion` → `SessionQuestionCard`)· `i18n/zh.ts:1368-1372` |
...
| 项 | 规则 |
| --- | --- |
| 位置 | 时间线内,活跃回合的**最后一行**;新到的正文 / 思考块 / 工具卡都插在它之前。随内容滚动,**不悬浮、不 sticky** |
| 寿命 | `activity === "running"` 的整段:从这条用户消息成为活跃回合起,到 `session_status` 回到 `idle` 止。六个子状态里没有一个让它消失(AC1) |
...
| 等你面 | 回合在等你时:换成 `.retry-card` 同族的琥珀底小卡(`--a-warning-subtle` 底、40% `--a-warning` 边、radius lg)。12px 暂停符号(两竖线,`--a-warning` 描边,**静止**)+ 主句(`--a-text`,medium)+ 点分隔 + 去向(`--a-text-tertiary`,xs)。**计时收起**。无按钮:动作仍在弹窗 / 提问卡里(AC3)。**两种触发共用这一面,只有文案不同**(§4 表) |
...
| 判据钩子 | `data-alpha-timeline-row="turn-running"`,`data-face="running" \| "waiting"`,等你面再标 `data-wait="approval" \| "question"` |
...
| ⑤ 等待你批准(`approvalPending`) | 「等待运行」灰 chip 的工具卡;弹窗在时间线之外 | **等你面**:脉冲停、琥珀、计时收起;「等待你的决定 · 生成已暂停,到审批窗口里选择」 |
| ⑥ 等待你回答(`question` 挂起) | 模型的问句正文;提问卡停靠在输入框上方 | **等你面,同一副面孔**;「等你回答 · 生成已暂停,到输入框上方的提问卡回答」 |

⑤ 与 ⑥ 是同一件事的两种来路 —— 回合停下来等你动手 —— 所以共用一面,不给第二种颜色或第二个符号,
只让那句话各说各的去向。「等待你的决定」是审批那句(composer placeholder 同源),反问不照抄。
两者同时挂起时(理论上审批先到)按审批说;实现票据实定优先级,不在本稿假设。

| 槽位 | zh | en |
| --- | --- | --- |
| 运行面主句 | 正在生成 | Generating |
| 等你批准 · 主句 | 等待你的决定 | Waiting for your decision |
| 等你批准 · 去向 | 生成已暂停,到审批窗口里选择 | Generation is paused — choose in the approval window |
| 等你回答 · 主句 | 等你回答 | Waiting for your answer |
| 等你回答 · 去向 | 生成已暂停,到输入框上方的提问卡回答 | Generation is paused — answer in the question card above the composer |
...
- 脚行的**文字部分**是 `role="status"`(隐含 `aria-live="polite"` + `aria-atomic="true"`),
  ...
  再加只会重复播报)。
- 只在值得听的时刻改字,因此整轮至多四次播报:出现时「正在生成」;等你批准「等待你的决定,生成已暂停,
  到审批窗口里选择」;等你回答「等你回答,生成已暂停,到输入框上方的提问卡回答」;你动完手回到「正在生成」。
  ①→②→③→④ 之间**不换字**,所以不播报 —— 那些变化由正文、思考块、工具卡进入 `role="log"` 时各自播报。
...
## 7. owner 裁决(2026-09-22)

递上去时三点各带默认值;owner 全按默认批,并加一条:

1. **「正在思考」胶囊并进脚行** —— 批。一轮里只有一个元素、一种形状;活稿 `②` 那枚胶囊帧保留并就地标注,
   首个字到达前的文案由「正在思考…」变为「正在生成」。
2. **带计时** —— 批。减弱动效下唯一的活信号,「没卡住」最诚实的证据,与完成后脚注里的时长首尾相接。
3. **不悬浮** —— 批。随内容滚动;滚上去读旧内容时它随回合离开视野,那时顶栏胶囊仍在。
4. **补画第六态「模型反问你」** —— owner 要求;并进等你面,同一副面孔、只换文案(§4)。

## 8. 落点与本稿明知不做的事

已做:帧并入 `current/conversation-timeline/design.html` 的 `②` 节末,锚 `#turn-running`;同段「正在思考」
胶囊帧就地标注「已并入回合脚行」;`⑦` 组件索引加行;样式落 `current/conversation-timeline/design.css`
(`.turnfoot` / `.turnfoot-wait`);台账行翻为「待实现」(设计定稿 2026-09-22)。实现票关闭的同一 PR 回填
落地列与代码入口;`#1399` 的 AC 若因第六态而措辞有变,**AC 改写归 owner**。本目录就地冻结。

**不做**:不动审批走独立弹窗、提问卡停靠输入框上方这两条既有形态;不动顶栏胶囊 —— 但记一条实读出来的
不一致:回合在等你时顶栏仍显示「正在生成…」,与本稿等你面相左,是否让胶囊也分两面归后续;不做进度
百分比与预计剩余。
