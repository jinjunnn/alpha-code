...
> ⚠️ **§3 / §12 / §14 的若干数字已被交付期实测订正 —— 见文末 §15，冲突以 §15 为准。**
> 已知被订正的：可装数 135→**132**、「40 个有技能的插件」→**37/38**、全灭 10 的**名单成员**、
> 两轴差 3→**1**、恒真式 3 处→**7 处**、§14 R2-b 的绕过配方在真实语料上不成立。
> 交付期另新增 owner 裁决一条（账本可选 `displayName`），见 §15。
...
## §15 实测订正（交付后回填 —— 与 §3 / §12 / §14 冲突时**以本节为准**）

Phase 3 五张票交付过程中，实现方与验证方**用真实语料实跑**核出本稿若干数字有误。
按「该查就别回忆 / 不许往基线上凑」的纪律，**订正记在这里，原文保留** ——
保留是为了留住「哪个数字错过、是怎么被发现的」这条线索，不是为了给它留后路。

**凡下游引用本稿数字者，一律引用本节。**

| 出处 | 原文 | 实测订正 | 怎么发现的 |
| --- | --- | --- | --- |
| §3.6 | 可装 **135 / 162（83.3%）** | **132** | 分母错：**应是 159 份「支持布局」的 SKILL.md，不是 162** —— 那 3 份异常布局正是 G18 具名拒绝的，结构上装不了。`132 = 159 − 27`。`ac#780` 首报，`ac#783` 加回归并**同时钉住 132 与「135 只是原算法的复现」** |
| §3.6 | 「**40** 个有技能的插件」 | **37**（intake 口径）/ **38**（路径归属口径）——**都不是 40** | `ac#783` 两轴实测 |
| §3.6 | 全灭 **10** 个插件的**名单** | **计数 10 正确，成员错**：含 `claude-for-msft-365-install`（实为 0 候选，属那 25 个 0-skill 插件），**漏了 `plugin-dev`**（7 个候选全被拒） | `ac#783` 逐个复算 |
| §3.2 | 「两轴差 **3**」 | 差 **1** | 两份 manifestless 的 SKILL.md **没有 `plugin.json` 祖先** —— 按本稿 §3.2 自己的表，它们**结构上无法**增加 owner 数 |
| §12 风险 6 / §14 | 恒真式 **3 处** | 实测 **7 处** | 多出四处：G18③ `workflow-skills`（记的 4 个实例**全在 `cache` 里**，而 §3.1 已把 `cache` 排除出判据语料）、G18⑥、G16 R2-a，以及 **G17 块式** |
| §14 R2-b | 「把检测口径改回『要求有标量值』⇒ 块式那一半**必须变红**」 | **该绕过配方在真实语料上不成立** | `ac#780` 实测：7 份块式文件**全都同时**带一个标量控制字段 ⇒ 技能级拒绝集**两种口径都是 12**。**修法本身仍必要**（管的是可达性，不是当前计数），但**只有「唯一控制字段是块式」的合成夹具杀得掉它** |
| §3.6 / §12 风险 4 | 连续安装场景「可装 135 → **56**」 | **未复核** —— 它由已被订正的 135 推导而来 | 本轮无人重测该场景。**引用前须重跑**，不许照抄 |
| §9 D3 一带 | （未记）`displayName` 这个字段当时不存在 | 交付期**新增** `PackageGraphV1.displayName?`（owner 裁决） | 见下「交付期新增的裁决」 |

### 恒真式多出的那个**形态**（比数目更值得记）

原稿把恒真式理解为「**真实语料 0 例** ⇒ 那半场恒真」。**G17 块式是另一个形态：
有例，但两种口径区分不出来。**

7 份块式文件全都同时带标量控制字段 ⇒ 无论按「键是否出现」还是按「有标量值」判，结果都是 12。
**你能看到真实语料在跑、也能看到它绿** —— 它比「0 例」**更容易被读成验过了**。

**判据因此要改写**：不是「这道闸的真实语料半场有没有例」，而是
**「把实现改成错的，这条断言会不会变红」**。答案是「不会」的就是恒真式，与有没有例无关。

### 交付期新增的裁决：`PackageGraphV1.displayName?`

owner 裁决（交付期）：账本加一个**可选**显示名字段，安装时存 `plugin.json` 自己声明的 `name`，
列表显示用户导入的那个插件名而不是包里某个技能名。**可选不必填**（存量图缺席合法，回退 root 组件名）；
**只管显示**，来源判定仍只看 child record 的 `origin`。

它自己长出过两个缺陷，都已修，**记在这里当作后来人的对照**：

1. **decoder 校验了它却没把它带进解码结果** ⇒ 摘要少一项 ⇒
   **一张合法的、带显示名的图被防篡改闸判成「被改过」而拒载**。
   ——「前提为假的闸门比没有闸更贵」的最小实例：它拒的不是坏输入，是我们自己刚写的配置。
2. **intake 与账本各写了一份显示名文法且不一致** ⇒ 超长名字**预览说能装、确认时 commit 期回滚**。
   修法：判据抽成 `isValidPackageDisplayName` 一处真源，intake / 安装器 / decoder 三方共用，
   **判定发生在预览期**；存不下就**不存**（缺席合法）并在确认前告知，**不截断**
   —— 截断等于把作者写的名字改写成我们编的名字。

### 交付实况

- **五张票全部合并**：`ac#780` intake / `ac#781` install / `ac#782` channel /
  `ac#784` renderer / `ac#783` verify（夹具半场）。主线 `alpha-code@9996d2c9`。
- **闸门绕过记录**见 `docs/verification/2026-08-02-req128-phase3-gate-bypass-matrix.md`
  （三类点名：有记录 / 无记录 / 恒真式）与各票的 `docs/verification/*-gate-bypass-experiments.md`。
- **§12 风险 5「零真机证据」的处置**：它**不是**遗留边界，而是 `ac#783` **AC⑤ 自身的内容**。
  该票在真机 L2 交付前保持 OPEN。**任何人不得因为夹具全绿而把它当作已闭合。**
