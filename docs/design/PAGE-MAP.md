...
| Conversation timeline / 时间线 | partial | `ui-mac/src/renderer/alpha-ui/timeline-inject.tsx` + `timeline/*.css` | [`current/conversation-timeline/`](current/conversation-timeline/design.html) | timeline-overhaul; artifact-rows 增量 = `2026-07-21-req124-timeline-artifact-rows/`(已批准;设计层已并入 current §⑥,2026-07-23;实现票 #449 open)。manifest target = alpha |
...
| Session workspace / 会话工作区 | partial | `ui-mac/src/renderer/alpha-ui/session-workspace/alpha-session-workspace.tsx` | [`current/session-workspace/`](current/session-workspace/design.html) | composer-model-redesign; req090. release default = alpha(外框已发布;内瓤替换进行中) |
...
- **Session workspace** (`route.session`) — frame released; upstream leaf being replaced.
