...
| Alpha Office connectors | [`req-133-office-four-format-hub.md`](req-133-office-four-format-hub.md) (accepted — four Alpha-authored local stdio wrappers, workspace sandbox, exact Hub commands) → [`req-134-mcp-workspace-follows-instance.md`](req-134-mcp-workspace-follows-instance.md) (accepted — durable `{workspace}`; spawn substitutes `InstanceState.directory`) → [`req-135-retire-community-excel.md`](req-135-retire-community-excel.md) (accepted — Hub Excel is Alpha-only; the community connector is retired) → [`req-155-office-connector-follows-instance.md`](req-155-office-connector-follows-instance.md) (accepted — installs follow the running instance: boot re-anchors a stale bundle path and adopts committed records from sibling channel roots, read-only) | current `packages/ui-mac/resources/office-mcp`, spawn substitution, boot retirement, boot instance reconcile, main MCP policy, and office advisories/tests |
| Model provider credentials | `2026-06-29-llm-auth-routing/design.md` (§0.4 auth 全归 alpha;BYOK key 存 safeStorage 钥匙串) → [`2026-09-17-req226-custom-provider-keychain-baseline.md`](2026-09-17-req226-custom-provider-keychain-baseline.md) (draft — 目录外自定义服务密钥并入同一钥匙串库与 A6 `{file:}` 通道;durable 配置只留常量标记,引用只在 fork 时注入) | `packages/ui-mac/src/main/alpha-byok-keys.ts`、`alpha-secret-files.ts`、`alpha-models.ts` 与 [`../contracts/engine-config-channels.md`](../contracts/engine-config-channels.md) |
