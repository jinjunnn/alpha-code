...
- **「风险与自认边界」清单不是免责声明,是攻击面清单**。交付方应在等审期间**逐条构造复现**
  自己登记的条目 —— 实例:某实现把上一轮登记为「理论风险」的一条构造出来,实测
  `reads=201 / broadcasts=1`(owner 既不采信也不停手),**比登记的严重**,当轮自修。
...
11. **论证用的性质本身必须是真的**。写「不可达 / 单调 / 一定不会」之前,先问**这个性质有没有闸门**。
    —— 实例:为论证某放大循环不可达,声称「token 生命周期内 `platformStatus` 单调」,
    但 `Date.now()` 跨期限与 `unappliedRenewalGeneration` 的置位/清除都能改变它,
    **且同一文件另一条不变量(D-4)正依赖该状态可翻转** —— 一边靠它能翻立规矩,一边靠它不翻免检。
    这一族与 ③″3-6 方向相反:**声称的比做到的更严**,而且**更隐蔽,因为它读起来像是更谨慎**;
    没人会去查一个否定断言的依据。
12. **源码里的标注才算数,报告里的不算**。「非闸门覆盖项」这类诚实标注若只写在交付报告里、
    没写进源码注释,下一个读代码的人看到的仍是那句不成立的声称。
    —— 实例:某判据在报告中已标「结构论证、非独立闸门」,而源码仍写着「any allow-rule edit fails」。
