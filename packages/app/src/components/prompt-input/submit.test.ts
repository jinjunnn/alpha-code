...
beforeAll(async () => {
  ...
  // pin e11dbd020:上游多了 `toaster` 等命名导出;整体替换命名空间会让单跑本文件在加载期死于
  // `Export named 'toaster' not found`(全量跑时别的文件先把真模块装进来了,所以只有单跑红)。
  const realToastModule = { ...(await import("@opencode-ai/ui/toast")) }
