...
import { persistProviderAndRefresh, removeProviderAndRefresh, setProviderKeyAndRetireLegacyKey } from "./provider-lifecycle"
...
export function registerProviderIpcHandlers() {
  ...
  // Re-entering a key also retires a pre-#1343 plaintext `options.apiKey` leaf for that id — only that leaf,
  // from whichever provider read path carries it (R1 finding 1 / R2 blocker).
