...
describe("finalizeArtifactWithQuota", () => {
  ...
  test("slow asynchronous reservation I/O cannot overrun the wait deadline or block the event loop", async () => {
    seedReservation({
      declaredBytes: 1,
      startedAt: String((RACE_STARTED_AT + 1_000) * 1_000),
    })
  ...
  test("a legal project above the 10,000-entry scan slice completes admission", async () => {
    const runCount = 41
    ...
    const result = await write("large-project.bin", "x", {
      limits: limits({ runMaxCount: 256, projectMaxBytes: 5 * 1024 * 1024 * 1024 }),
    })

    expect(result.ok).toBe(true)
    expect(existsSync(target("large-project.bin"))).toBe(true)
    expect(reservations()).toEqual([])
    expect(stagingResidue()).toEqual([])
  })
