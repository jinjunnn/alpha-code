...
//    CI 输出里读不到的降级等于没降级。落进**后者**的测试文件则必须进 `scripts/gate-files.tsv`:
//    这张表说它是真闸门,而真闸门不许「删掉不红」(把文件连同表里那一行一起删掉,⑤ 的其余几条
//    全绿、第 ① 层对这些名字失明、整包地板吸收得掉)。
...
describe("闸门文件登记簿完备性 (#647)", () => {
  ...
  test("⑤ 被判为「文本就是产物」的测试文件必须登记进 gate-files.tsv(本表亲手认证的真闸门不许删掉不红)", () => {
    // 这一条治的是本层自己的一个洞:`KEPT_SOURCE_TEXT_READS` 的公开含义是「文本就是被守的产物,
    // 不降级」——也就是**这张表亲自认证它是真闸门**。而第 ① 层按文件名正则发现闸门,对这些名字
    // 多数失明;第 ⑤ 层其余五条只要求「表里的路径今天仍命中」,把文件连同表里那一行**一起**删掉
    // 全部保持绿,整包地板也吸收得掉(几条到几十条,余量 1000+)。
    // ⇒ 认证成真闸门,就得拿真闸门的保护:进登记簿,吃精确条数。
    // `.cases.ts` 键豁免 —— 它们本来就不进整包计数,由第 ④ 层反向那半(每个 cases 文件必须被一个
    // **已登记**宿主跑到)兜住,再进本表会与那一层重复登记同一件事。
    const unprotected = Object.keys(KEPT_SOURCE_TEXT_READS).filter(
      (path) => /\.(test|spec)\.tsx?$/.test(path) && !REGISTERED.includes(path),
    )
    expect(
      unprotected,
      [
        "下列文件被登记为「文本就是被守的产物(不降级)」,却没有登记进 scripts/gate-files.tsv。",
        "两件事只能选一件,不许都要:",
        "  · 它真是闸门 → 加一行进 scripts/gate-files.tsv(delegates_to 通常是 '-'),再跑",
        "    `bash scripts/assert-gate-files.sh --update` 把精确条数写回。",
        "  · 它其实是减速带 → 挪进 DOWNGRADED_ANCHORS 并把测试标题改成 `ANCHOR (not a gate): …`。",
        "留在本表而不进登记簿 = 这张表说它是真闸门,而删掉它(连同表里那一行)不会有任何东西变红。",
      ].join("\n"),
    ).toEqual([])
  })
