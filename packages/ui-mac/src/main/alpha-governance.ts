...
export const ALPHA_INJECTED_AGENTS = ["alpha-automation"] as const
...
export function materializeEdits(gov: Governance, visibleAgents: string[]): GovernanceEdit[] {
  ...
  if (gov.skills.deny.length) {
    ...
    for (const n of gov.skills.deny) {
      ...
      // 斜杠泄漏缓解(诚实优先):同名 command 占位模板 —— 菜单里仍可见但点开是诚实说明,非误导执行
      edits.push({ path: ["command", n, "description"], value: `(已禁用)该技能已在 alpha 治理中禁用` })
      edits.push({ path: ["command", n, "template"], value: `该技能(${n})已在 alpha 的治理设置中被禁用。请告知用户:此技能不可用;如需恢复,到 定制中心 → 已安装 → 内置(上游) 解除禁用。不要尝试其它方式执行该技能。` })
    }
  }
