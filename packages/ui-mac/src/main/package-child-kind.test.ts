...
//   · **只写 A(五个都在表里)不够** —— 一个把 `else` 留在那里的实现对已登记的五个都对,
//     对第六个静默变 `mcp`。
//
...
describe("package child kind 路由表 (#809 · 基线 §5 第 7 类)", () => {
  ...
  test("表的键集恰等于 PROFILE_REGISTRY_V1 的 profileId 集(期望值从注册表派生)", () => {
    ...
    expect(registered.length).toBeGreaterThanOrEqual(5) // 前提自检:注册表读空时不能静默通过
  ...
  // ── B:未登记即拒 ────────────────────────────────────────────────────────────
  test("未登记的第六个 profile 被具名拒绝,而不是静默变成 mcp", () => {
    for (const unknown of ["opencode-tui-plugin", "cloud", "bundle", "", "MCP-LOCAL"]) {
