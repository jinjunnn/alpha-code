...
The `32` is `limits.maxComponents` in the registry; both `maxItems` values in the
envelope schema (`components`, and `dependencies` at `maxComponents - 1`) are
derived from it. See the `maxComponents` doc comment in `registry.ts` for the
counting basis and the derivation.
...
sideloaded legacy extension, but they are not host package capabilities.
...
token grammar is a length/character bound rather than an admission gate. No
registered token contains a colon, and the grammar admits none.
...
policy, nested (depth > 1) graphs, version solving, or any profile whose payload
is code the engine evaluates in its own process (ADR-040).
