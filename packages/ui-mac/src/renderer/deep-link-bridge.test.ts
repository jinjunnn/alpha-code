...
import { publishDeepLinks, type DeepLinkBufferTarget } from "./deep-link-bridge"
...
describe("shell → renderer deep-link bridge", () => {
  ...
  test("deliveries that land before the layout mounts accumulate and drain together, once", () => {
    const t = target()
    publishDeepLinks(t.buffer, [OPEN_PROJECT])
    publishDeepLinks(t.buffer, [NEW_SESSION])
