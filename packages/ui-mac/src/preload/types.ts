...
export type CloudUploadResult =
  ...
  | {
      ...
      error:
        ...
        // unauthorized / network / http-<status> 回退,经 main 的 platform-error-code 咽喉
        // 原样到达 renderer。类型层如实退化为 string(它本来就不是安全边界);renderer 分流
        // 用结构判据(本地准入码全是 `upload-` 前缀 kebab)而不是手抄名单。
        | (string & {})
    }
