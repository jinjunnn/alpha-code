...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `${PRODUCT} is now connected to ${escapeHtml(provider)}.`
        : `${PRODUCT} is now authorized.`,
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `${PRODUCT} couldn't finish connecting to ${escapeHtml(provider)}.`
        : `${PRODUCT} couldn't complete authorization.`,
      detail,
      footnote: `Close this window and try again from ${PRODUCT}.`,
    }),
  })
}
